/*******************************************************************************
 * UDukeSceneMPPrivateMatch generated by Eliot.UELib using UELib.CLI.
...
function Created()
{
	...
	super.Created();
	// End:0x2B
	if(__NFUN_339__(MapList, none))
	{
		MapList = GetPlayerOwner().__NFUN_615__(class'MPMapInfo');
	}
...
function PopulateMutatorList()
{
	...
	{
		__NFUN_396__(class'MPMapInfo'.default.MutatorTypes[i].EntryName, PackageName, objName, DlcName);
		// End:0xC0
		if(__NFUN_308__(class'MPMapInfo'.default.MutatorTypes[i].EntryName, "None"))
		{
			LocalizedMutatorName = MutatatorNoneText;			
		}
		else
		{
			LocalizedMutatorName = __NFUN_356__(objName, "MutatorName", PackageName);
		}
		bRejected = false;
		ii = 0;
		J0xEF:
		...
		// End:0x187 [Loop If]
		if(__NFUN_169__(ii, string(class'MPMapInfo'.default.GameTypes[ModeSelect.GetSelectedIndex()].ExcludedMutators)))
		{
			// End:0x17D
			if(__NFUN_308__(class'MPMapInfo'.default.GameTypes[ModeSelect.GetSelectedIndex()].ExcludedMutators[ii], class'MPMapInfo'.default.MutatorTypes[i].EntryName))
			{
				bItemRejected = true;
				bRejected = true;
				// [Explicit Break]
	...
	// End:0x200
	if(__NFUN_148__(bItemRejected, __NFUN_170__(Index, 1)))
	{
	...
	{
		// End:0x26A
		if(__NFUN_150__(__NFUN_150__(__NFUN_150__(__NFUN_145__(bWasAReject), __NFUN_148__(bItemRejected, __NFUN_171__(Index, 1))), __NFUN_148__(bWasAReject, __NFUN_170__(Index, 0))), __NFUN_148__(bWasAReject, __NFUN_200__(__NFUN_211__(float(__NFUN_166__(MutatorSelection, Index))), float(2)))))
		{
			MutatorSelection = Index;
		}
...
function UpdateFields(UWindowDialogControl SliderChanged)
{
	...
	switch(SliderChanged)
	{
		// End:0x52
		case MapSelect:
			MapSelect.SetHelpText(__NFUN_356__("MapHintText", MapSelect.SelectedItem.Value, "Maps"));
			// End:0x1BA
			break;
		// End:0x165
		case ModeSelect:
			ModeSelect.SetHelpText(__NFUN_356__("GameModeHintText", ModeSelect.SelectedItem.Value, "dnWindow"));
			...
			// End:0x141
			if(__NFUN_171__(numPlayerOffset, 0))
			{
				numPlayerOffset = 1;
			}
		...
		// End:0x1B7
		case MutatorSelect:
			MutatorSelect.SetHelpText(__NFUN_356__("MutatorHintText", MutatorSelect.SelectedItem.Value, "dnWindow"));
			// End:0x1BA
			break;
		// End:0xFFFF
		default:
...
{
	super.Paint(C, X, Y);
	ButtonHeight = int(__NFUN_195__(float(33), WinScaleY));
	ButtonWidth = int(__NFUN_195__(float(491), WinScaleY));
	ButtonLeft = int(__NFUN_195__(float(78), WinScaleY));
	...
	MapSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	ModeSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	MutatorSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	MaxPlayerSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	MaxScoreSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	TimeLimitSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	RotationTypeSelect.ArrowLeft = __NFUN_195__(LeftArrowPlacement, WinScaleY);
	ModeSelect.WinLeft = float(ButtonLeft);
	ModeSelect.WinTop = float(ControlStart);
	MapSelect.WinLeft = float(ButtonLeft);
	MapSelect.WinTop = __NFUN_198__(__NFUN_198__(ModeSelect.WinTop, ModeSelect.WinHeight), float(ControlBuffer));
	MutatorSelect.WinLeft = float(ButtonLeft);
	MutatorSelect.WinTop = __NFUN_198__(__NFUN_198__(MapSelect.WinTop, MapSelect.WinHeight), float(ControlBuffer));
	MaxPlayerSelect.WinLeft = float(ButtonLeft);
	MaxPlayerSelect.WinTop = __NFUN_198__(__NFUN_198__(MutatorSelect.WinTop, MutatorSelect.WinHeight), float(ControlBuffer));
	MaxScoreSelect.WinLeft = float(ButtonLeft);
	MaxScoreSelect.WinTop = __NFUN_198__(__NFUN_198__(MaxPlayerSelect.WinTop, MaxScoreSelect.WinHeight), float(ControlBuffer));
	TimeLimitSelect.WinLeft = float(ButtonLeft);
	TimeLimitSelect.WinTop = __NFUN_198__(__NFUN_198__(MaxScoreSelect.WinTop, MaxScoreSelect.WinHeight), float(ControlBuffer));
	RotationTypeSelect.WinLeft = float(ButtonLeft);
	RotationTypeSelect.WinTop = __NFUN_198__(__NFUN_198__(TimeLimitSelect.WinTop, TimeLimitSelect.WinHeight), float(ControlBuffer));
	PrivateFlagBox.WinLeft = float(ButtonLeft);
	PrivateFlagBox.WinTop = __NFUN_198__(__NFUN_198__(RotationTypeSelect.WinTop, RotationTypeSelect.WinHeight), float(ControlBuffer));
	...
	CreateGameBttn.WinTop = __NFUN_198__(__NFUN_198__(PrivateFlagBox.WinTop, PrivateFlagBox.WinHeight), float(ControlBuffer));
	return;
}
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
	...
	switch(E)
	{
		...
		case 2:
			// End:0x5E
			if(__NFUN_339__(C, CreateGameBttn))
			{
...
function PopulateGameTypeList()
{
	...
	{
		// End:0xBE
		if(__NFUN_145__(class'MPMapInfo'.default.GameTypes[i].bUIExclude))
		{
			__NFUN_396__(class'MPMapInfo'.default.GameTypes[i].EntryName, PackageName, objName, DlcName);
			ModeSelect.AddItem(__NFUN_356__(objName, "GameName", PackageName), class'MPMapInfo'.default.GameTypes[i].EntryName);
		}
		__NFUN_182__(i);
		// [Loop Continue]
		goto J0x17;
...
function int PopulateMapList(optional string DefaultMap)
{
	...
	// End:0xAD [Loop If]
	if(__NFUN_169__(i, string(mapnames)))
	{
		LocalizedMapName = __NFUN_356__("MapNames", mapnames[i].MapName, "Maps");
	...
	// End:0xD3
	if(__NFUN_309__(DefaultMap, ""))
	{
		return __NFUN_189__(0, MapSelect.FindItemByText(DefaultMap));
	}
	return 0;
	return;
}
...
function PopulateMaxScoreList()
{
	...
	if(MapList.GetOptionsForGameType(ModeSelect.SelectedItem.Value, GameTypeScoreSettings))
	{
		MaxScoreSelect.SetText(__NFUN_356__("UDukeSceneMPPrivateMatch", GameTypeScoreSettings.WinConditionText, "dnWindow"));
		maxScore = GameTypeScoreSettings.WinConditionMin;
		J0x92:

		// End:0xD0 [Loop If]
		if(__NFUN_171__(maxScore, GameTypeScoreSettings.WinConditionMax))
		{
			MaxScoreSelect.AddItem(string(maxScore));
			__NFUN_180__(maxScore, GameTypeScoreSettings.WinConditionStep);
			// [Loop Continue]
			goto J0x92;
		}
...
function CreatePressed()
{
	...
	// End:0x33D
	if(__NFUN_308__(RotationTypeSelect.SelectedItem.Value, RotationTypeRandomValue))
	{
		__NFUN_186__(GetPlayerOwner().__NFUN_612__());
		MapList.GetMapsForGameType(ModeSelect.SelectedItem.Value, mapnames);
		i = 0;
		J0xF5:
		...
		{
			// End:0x137
			if(__NFUN_308__(mapnames[i].MapName, MapSelect.SelectedItem.Value))
			{
				// [Explicit Break]
				goto J0x141;
			}
		...
		// End:0x158
		if(__NFUN_172__(i, string(mapnames)))
		{
			i = 0;
		}
		...
		// End:0x33A [Loop If]
		if(__NFUN_170__(string(mapnames), 0))
		{
			randNum = __NFUN_187__(string(mapnames));
	...
	TimeLimit = __NFUN_162__(int(TimeLimitSelect.SelectedItem.Text), 60);
	// End:0x4BA
	if(__NFUN_308__(MaxScoreSelect.SelectedItem.Value, ""))
	{
		UDukeRootWindow(Root).SetAdditionalOptions(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__("?", MaxScoreURLText), "="), MaxScoreSelect.SelectedItem.Text), "?RoundTimeLimit="), string(TimeLimit)));		
	}
	else
	{
		UDukeRootWindow(Root).SetAdditionalOptions(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__("?", MaxScoreURLText), "="), MaxScoreSelect.SelectedItem.Value), "?RoundTimeLimit="), string(TimeLimit)));
	}
...
function Tick(float Delta)
{
	super.Tick(Delta);
	// End:0x6A
	if(__NFUN_148__(__NFUN_145__(__NFUN_402__()), __NFUN_145__(UDukeRootWindow(Root).AgentOnline.IsSignedIn())))
	{
		NavigateBack();
