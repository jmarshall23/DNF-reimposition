/*******************************************************************************
 * DukeDialogBoxManager generated by Eliot.UELib using UELib.CLI.
...
final function ShowCustomDialogBox(optional string Title, optional string body, optional MaterialEx image)
{
	...
	DialogStack[__NFUN_166__(string(DialogStack), 1)].id = 2;
	DialogStack[__NFUN_166__(string(DialogStack), 1)].CBObj = none;
	DialogStack[__NFUN_166__(string(DialogStack), 1)].secretString = "";
	return;
}
...
final function UDukeSuperMessageBox ShowDialogBox(Engine.Object.EConsole_Dialog dialogID, optional UWindowWindow CBObj, optional string secretKey, optional class<UDukeSuperMessageBox> derivedClass, optional bool bPlayOpenedSound)
{
 ...
 We support newlines?", OKStr, CancelStr);
		...
		// End:0xF19
		case 66:
			FormattingArray.Remove(0, __NFUN_166__(string(FormattingArray), 1));
			...
			// End:0xED1 [Loop If]
			if(__NFUN_169__(Idx, string(ExtraStringArray)))
			{
	...
	DialogStack[DialogStack.Add(1)].msgBox = newBox;
	DialogStack[__NFUN_166__(string(DialogStack), 1)].id = dialogID;
	DialogStack[__NFUN_166__(string(DialogStack), 1)].CBObj = CBObj;
	DialogStack[__NFUN_166__(string(DialogStack), 1)].secretString = secretKey;
...
final function DukeMessageBoxDone(UDukeSuperMessageBox returnBox, int Result)
{
	...
	// End:0xAB [Loop If]
	if(__NFUN_169__(i, string(DialogStack)))
	{
		// End:0xA1
...
final function CloseWaitingDialog()
{
	...
	if(__NFUN_169__(i, string(DialogStack)))
	{
		...
		// End:0xC3
		if(__NFUN_173__(int(DialogStack[i].id), int(3)))
		{
...
final function CloseAllWaitingDialogs()
{
	...
	{
		// End:0x69
		if(__NFUN_150__(__NFUN_173__(int(DialogStack[i].id), int(3)), __NFUN_173__(int(DialogStack[i].id), int(4))))
		{
	...
	// End:0xE2 [Loop If]
	if(__NFUN_169__(i, string(MessageBoxes)))
	{
...
function CloseDialogBoxByID(Engine.Object.EConsole_Dialog dialogID)
{
	...
	{
		// End:0x4C
		if(__NFUN_173__(int(DialogStack[i].id), int(dialogID)))
		{
			msgBox = DialogStack[i].msgBox;
			// [Explicit Break]
			goto J0x56;
...
function UDukeSuperMessageBox CreateMsgBox(Engine.Object.EConsole_Dialog boxID, optional class<UDukeSuperMessageBox> classToMake)
{
	...
	// End:0xAC [Loop If]
	if(__NFUN_169__(i, string(DialogCache)))
	{
		// End:0xA2
		if(__NFUN_148__(__NFUN_148__(__NFUN_173__(int(DialogCache[i].DialogType), int(boxID)), __NFUN_339__(DialogCache[i].Class, classToMake)), __NFUN_145__(DialogCache[i].WindowIsVisible())))
		{
...
function sortByDrawOrder()
{
	...
	// End:0xC8 [Loop If]
	if(__NFUN_169__(i, __NFUN_166__(Length, 1)))
	{
		j = __NFUN_165__(i, 1);
		J0x33:

		// End:0xBE [Loop If]
		if(__NFUN_169__(j, Length))
		{
			// End:0xB4
			if(__NFUN_170__(DialogStack[i].msgBox.iDrawPriority, DialogStack[j].msgBox.iDrawPriority))
			{
				tmp = DialogStack[i];
...
function DrawDialogs(Canvas C)
{
	...
	// End:0x15C
	if(__NFUN_170__(string(DialogStack), 0))
	{
	...
	bInDigs = __NFUN_148__(__NFUN_340__(dukeCon.Viewport.Actor.Level.Game, none), dukeCon.Viewport.Actor.Level.Game.__NFUN_358__('dnMyDigs'));
	// End:0x215
	if(__NFUN_148__(__NFUN_170__(string(DialogStack), 0), __NFUN_145__(dukeCon.bDontDrawMouse)))
	{
...
final function PaintClients(Canvas C, float X, float Y, UDukeSuperMessageBox msgBox)
{
	...
	if(true)
	{
		C.OrgX = __NFUN_198__(C.OrgX, __NFUN_195__(Child.WinLeft, dukeCon.Root.GUIScale));
		C.OrgY = __NFUN_198__(C.OrgY, __NFUN_195__(Child.WinTop, dukeCon.Root.GUIScale));
		// End:0x4FD
		if(__NFUN_145__(Child.bNoClip))
		{
			C.ClipX = __NFUN_195__(__NFUN_224__(__NFUN_199__(WinWidth, Child.WinLeft), Child.WinWidth), dukeCon.Root.GUIScale);
			C.ClipY = __NFUN_195__(__NFUN_224__(__NFUN_199__(WinHeight, Child.WinTop), Child.WinHeight), dukeCon.Root.GUIScale);
...
function bool WindowEvents(UWindow.UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
	...
	{
		// End:0x4A
		if(__NFUN_174__(int(msg), int(10)))
		{
...
function UDukeSuperMessageBox GetTopWindow()
{
	...
	if(__NFUN_170__(string(DialogStack), 0))
	{
		return DialogStack[__NFUN_166__(string(DialogStack), 1)].msgBox;
	}
	return none;
	return;
}
...
{
	return __NFUN_174__(string(DialogStack), 0);
	return;
}
