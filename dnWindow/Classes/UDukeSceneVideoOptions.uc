/*******************************************************************************
 * UDukeSceneVideoOptions generated by Eliot.UELib using UELib.CLI.
...
function Created()
{
	...
	if(__NFUN_414__())
	{
		...
		{
			Brightness.AddItem(string(i), string(__NFUN_195__(float(i), 0.1)));
			__NFUN_184__(i);
			// [Loop Continue]
			goto J0x143;
		}		
	...
	else
	{
		...
		if(__NFUN_171__(i, 10))
		{
			Brightness.AddItem(string(i), string(__NFUN_198__(__NFUN_196__(__NFUN_196__(float(i), 10), 2), 0.25)));
			__NFUN_184__(i);
			// [Loop Continue]
			goto J0x187;
		}
...
function LoadAvailableResolutions()
{
	...
	if(FullscreenCheck.bChecked)
	{
		...
		// End:0xFD [Loop If]
		if(__NFUN_169__(DisplayModeIndex, NumDisplayModes))
		{
			// End:0xC4
			if(__NFUN_150__(__NFUN_145__(GetPlayerOwner().__NFUN_989__(AspectIndex, DisplayModeIndex, Mode.Width, Mode.Height, Mode.Refresh)), __NFUN_169__(Mode.Height, 480)))
			{
				// [Explicit Continue]
				goto J0xF3;
			}
...
function AdjustAspectRatioSlider()
{
	...
	{
		// End:0xC9
		if(__NFUN_173__(AspectRatio.GetSelectedIndex(), 4))
		{
...
function int GetCurrentAspectIndex()
{
	...
	ASPECT_EPSILON = 0.01;
	AdjustAspectRatioSlider();
	Aspect = __NFUN_196__(float(CurrentResX), float(CurrentResY));
	// End:0x49
	if(__NFUN_200__(__NFUN_211__(__NFUN_199__(Aspect, __NFUN_196__(4, 3))), ASPECT_EPSILON))
	{
		return 0;		
	}
	else
	{
		// End:0x6D
		if(__NFUN_202__(__NFUN_211__(__NFUN_199__(Aspect, __NFUN_196__(5, 4))), ASPECT_EPSILON))
		{
			return 1;			
		}
		else
		{
			// End:0x92
			if(__NFUN_202__(__NFUN_211__(__NFUN_199__(Aspect, __NFUN_196__(16, 9))), ASPECT_EPSILON))
			{
				return 2;				
			}
			else
			{
				// End:0xB4
				if(__NFUN_202__(__NFUN_211__(__NFUN_199__(Aspect, __NFUN_196__(16, 10))), ASPECT_EPSILON))
				{
					return 3;
				}
			}
...
function LoadCurrentSettings()
{
	...
	else
	{
		...
		{
			// End:0x17F
			if(__NFUN_202__(__NFUN_211__(__NFUN_199__(float(localItem.Value), BrightnessValue)), 0.005))
			{
	...
	FullscreenCheck.bChecked = __NFUN_150__(__NFUN_414__(), GetPlayerOwner().Level.__NFUN_1165__());
	...
	// End:0x36F
	if(__NFUN_145__(FullscreenCheck.bChecked))
	{
...
function AspectRatioChanged()
{
	...
	bNeedResChange = true;
	// End:0x67
	if(__NFUN_173__(GetCurrentAspectIndex(), int(AspectRatio.SelectedItem.Value)))
	{
		Resolution.SetSelectedIndex(0);
		// End:0x64
	...
	{
		// End:0xAC
		if(__NFUN_148__(__NFUN_173__(int(AspectRatio.SelectedItem.Value), 4), __NFUN_145__(FullscreenCheck.bChecked)))
		{
			bNeedResChange = false;
			return;			
		}
	...
	switch(E)
	{
		...
		case 2:
			switch(C)
			{
				...
				case AdvancedButton:
					// End:0xF5
					if(__NFUN_148__(bNeedResChange, __NFUN_145__(__NFUN_414__())))
					{
						NewSettings = Resolution.SelectedItem.Text;
						// End:0xF2
						if(__NFUN_309__(NewSettings, CurrentRes))
						{
							bGoingBack = false;
...
function Paint(Canvas C, float X, float Y)
{
	super.Paint(C, X, Y);
	// End:0x2E
	if(__NFUN_148__(__NFUN_145__(__NFUN_414__()), bRefreshRes))
	{
		LoadCurrentSettings();
	}
	// End:0x6A
	if(__NFUN_150__(__NFUN_339__(ChildInFocus, AspectRatio), __NFUN_339__(ChildInFocus, Resolution)))
	{
	...
	Brightness.ArrowLeft = __NFUN_199__(float(ButtonWidth), __NFUN_195__(float(200), WinScaleY));
	AspectRatio.ArrowLeft = __NFUN_199__(float(ButtonWidth), __NFUN_195__(float(200), WinScaleY));
	Resolution.ArrowLeft = __NFUN_199__(float(ButtonWidth), __NFUN_195__(float(200), WinScaleY));
	// End:0x24E
	if(FullscreenCheck.bChecked)
	{
	...
	else
	{
		...
		if(FullscreenCheck.bChecked)
		{
			AspectRatio.WinTop = __NFUN_198__(__NFUN_198__(Brightness.WinTop, Brightness.WinHeight), float(ControlBuffer));			
		}
		else
		{
		...
		Resolution.WinTop = __NFUN_198__(__NFUN_198__(AspectRatio.WinTop, AspectRatio.WinHeight), float(ControlBuffer));
		FullscreenCheck.WinLeft = float(ButtonLeft);
		FullscreenCheck.WinTop = __NFUN_198__(__NFUN_198__(Resolution.WinTop, Resolution.WinHeight), float(ControlBuffer));
		VSyncCheck.WinLeft = float(ButtonLeft);
		VSyncCheck.WinTop = __NFUN_198__(__NFUN_198__(FullscreenCheck.WinTop, FullscreenCheck.WinHeight), float(ControlBuffer));
		AdvancedButton.WinLeft = float(ButtonLeft);
		AdvancedButton.WinTop = __NFUN_198__(__NFUN_198__(VSyncCheck.WinTop, VSyncCheck.WinHeight), float(ControlBuffer));
	}
	// End:0x49D
	if(__NFUN_148__(__NFUN_339__(ChildInFocus, Brightness), __NFUN_145__(Brightness.bDisabled)))
	{
...
function DrawBrightnessCalibration(Canvas C)
{
	...
	BoxWidth = __NFUN_195__(300, WinScaleY);
	BoxHeight = __NFUN_195__(300, WinScaleY);
	BoxLeft = float(__NFUN_165__(LineLeft, LineWidth));
	...
	numLines = WrapClipText(C, __NFUN_198__(BoxLeft, __NFUN_195__(float(10), WinScaleY)), __NFUN_198__(BoxTop, __NFUN_195__(float(10), WinScaleY)), VisibleHeaderText, false, int(__NFUN_199__(BoxWidth, __NFUN_195__(float(20), WinScaleY))), true, true, TTFontScale, TTFontScale);
	NormalSizeTextHeight = __NFUN_198__(__NFUN_198__(__NFUN_198__(__NFUN_198__(20, __NFUN_195__(YL, float(numLines))), YPos), __NFUN_195__(YL, float(4))), YL);
	// End:0x166
	if(__NFUN_201__(NormalSizeTextHeight, BoxHeight))
	{
		__NFUN_207__(TextScaleToFit, __NFUN_196__(BoxHeight, NormalSizeTextHeight));
	}
	C.DrawColor = GreyColor;
	TextSize(C, VisibleHeaderText, XL, YL, TextScaleToFit, TextScaleToFit);
	numLines = WrapClipText(C, __NFUN_198__(BoxLeft, __NFUN_195__(float(10), WinScaleY)), __NFUN_198__(BoxTop, __NFUN_195__(float(10), WinScaleY)), VisibleHeaderText, false, int(__NFUN_199__(BoxWidth, __NFUN_195__(float(20), WinScaleY))), false, true, TextScaleToFit, TextScaleToFit);
	YPos = __NFUN_198__(__NFUN_198__(__NFUN_198__(BoxTop, float(10)), YL), __NFUN_195__(YL, float(numLines)));
	C.DrawColor = GetPlayerOwner().NewColorBytes(175, 175, 175);
	TextSize(C, ClearlyVisible, XL, YL, TextScaleToFit, TextScaleToFit);
	ClipText(C, __NFUN_198__(BoxLeft, __NFUN_196__(__NFUN_199__(BoxWidth, XL), 2)), YPos, ClearlyVisible, false, TextScaleToFit, TextScaleToFit);
	C.DrawColor = GetPlayerOwner().NewColorBytes(45, 45, 45);
	TextSize(C, Visible, XL, YL, TextScaleToFit, TextScaleToFit);
	ClipText(C, __NFUN_198__(BoxLeft, __NFUN_196__(__NFUN_199__(BoxWidth, XL), 2)), __NFUN_198__(YPos, __NFUN_195__(YL, 1.5)), Visible, false, TextScaleToFit, TextScaleToFit);
	C.DrawColor = GetPlayerOwner().NewColorBytes(20, 20, 20);
	TextSize(C, BarelyVisible, XL, YL, TextScaleToFit, TextScaleToFit);
	ClipText(C, __NFUN_198__(BoxLeft, __NFUN_196__(__NFUN_199__(BoxWidth, XL), 2)), __NFUN_198__(YPos, __NFUN_195__(YL, 3)), BarelyVisible, false, TextScaleToFit, TextScaleToFit);
	return;
}
...
function Tick(float DeltaTime)
{
	...
	bFullScreen = __NFUN_150__(__NFUN_414__(), bool(GetPlayerOwner().ConsoleCommand("get ini:Engine.Engine.ViewportManager StartupFullscreen")));
	// End:0x86
	if(__NFUN_150__(bPendingReloadSettings, __NFUN_147__(bFullScreen, FullscreenCheck.bChecked)))
	{
		LoadCurrentSettings();
	}
	Brightness.bDisabled = __NFUN_145__(FullscreenCheck.bChecked);
	// End:0xD4
	...
	// End:0x11B
	if(__NFUN_201__(SwapDelayTimer, float(0)))
	{
		__NFUN_210__(SwapDelayTimer, DeltaTime);
		// End:0x11B
...
function WindowEvent(UWindow.UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
	...
	if(__NFUN_173__(int(msg), int(7)))
	{
		...
		{
			// End:0xD4
			if(__NFUN_150__(__NFUN_173__(Key, int(Root.Console.13)), __NFUN_173__(Key, int(Root.Console.210))))
			{
				// End:0xD4
				if(__NFUN_339__(ModalWindow, none))
				{
					// End:0xD4
					if(__NFUN_150__(__NFUN_339__(ChildInFocus, Resolution), __NFUN_339__(ChildInFocus, AspectRatio)))
					{
						AllowPageChange();
					}
				}
