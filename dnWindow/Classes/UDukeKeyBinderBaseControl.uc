/*******************************************************************************
 * UDukeKeyBinderBaseControl generated by Eliot.UELib using UELib.CLI.
...
{
	return __NFUN_166__(int(__NFUN_196__(WinHeight, KeyHeight)), 1);
	return;
}
...
{
	// End:0x1C
	if(__NFUN_148__(__NFUN_172__(i, 200), __NFUN_171__(i, 215)))
	{
		return true;
	}
	return false;
...
{
	return __NFUN_188__(int(__NFUN_196__(Y, KeyHeight)), GetNumDisplayed());
	return;
}
...
function Tick(float Delta)
{
	...
	{
		// End:0x42
		if(__NFUN_173__(SelectedIndex, i))
		{
			__NFUN_209__(SelectedTime[i], Delta);			
		}
		else
		{
			__NFUN_210__(SelectedTime[i], __NFUN_195__(2, Delta));
		}
		SelectedTime[i] = __NFUN_227__(SelectedTime[i], 0, class'UWindowScene'.default.SelectionTransitionTime);
		__NFUN_184__(i);
		// [Loop Continue]
		goto J0x12;
	}
...
function Paint(Canvas C, float X, float Y)
{
	...
	KeyHeight = __NFUN_195__(default.KeyHeight, class'UWindowScene'.default.TTFontScale);
	C.Font = C.TallFont;
	Column1X = int(__NFUN_195__(float(25), WinScaleY));
	Column2X = int(__NFUN_195__(float(320), WinScaleY));
	i = 0;
	J0x65:
	...
	if(__NFUN_169__(i, 150))
	{
		...
		if(__NFUN_173__(SelectedIndex, i))
		{
			...
			class'UWindowScene'.static.DrawSelectionIcon(C, __NFUN_195__(-8, C.FixedScale), __NFUN_199__(__NFUN_198__(__NFUN_198__(float(YOffset), __NFUN_195__(0.5, KeyHeight)), OrgY), C.OrgY), C.FixedScale, 1);
			C.__NFUN_1251__(OrgX, OrgY);
			C.__NFUN_1252__(ClipX, ClipY);
		}
		SelectedAlpha = __NFUN_196__(SelectedTime[i], class'UWindowScene'.default.SelectionTransitionTime);
		TextScaleX = __NFUN_228__(SelectedAlpha, class'UWindowScene'.default.TTFontScale, __NFUN_195__(class'UWindowScene'.default.SelectedXScale, class'UWindowScene'.default.TTFontScale));
		TextScaleY = __NFUN_228__(SelectedAlpha, class'UWindowScene'.default.TTFontScale, __NFUN_195__(class'UWindowScene'.default.SelectedYScale, class'UWindowScene'.default.TTFontScale));
		...
		ClipText(C, float(Column1X), __NFUN_198__(float(YOffset), __NFUN_196__(__NFUN_199__(KeyHeight, YL), 2)), Commands[i], false, TextScaleX, TextScaleY,, 2);
		// End:0x3FE
		if(__NFUN_148__(__NFUN_173__(SelectedIndex, i), bPolling))
		{
			C.DrawColor = class'UWindowScene'.default.RedColor;
			ClipText(C, float(Column2X), __NFUN_198__(float(YOffset), __NFUN_196__(__NFUN_199__(KeyHeight, YL), 2)), "?", false, class'UWindowScene'.default.TTFontScale, class'UWindowScene'.default.TTFontScale,, 2);			
		}
		else
		{
			DrawKey(C, float(Column2X), float(YOffset), i);
		}
		__NFUN_180__(YOffset, int(KeyHeight));
		__NFUN_184__(i);
		// [Loop Continue]
		goto J0x65;
	}
...
function KeyHold(int Key, float X, float Y)
{
	...
	switch(Key)
	{
		...
		case int(P.38):
			...
			SelectItem(__NFUN_166__(SelectedIndex, 1));
		...
		case int(P.40):
			...
			SelectItem(__NFUN_165__(SelectedIndex, 1));
...
{
	// End:0x1B
	if(__NFUN_150__(__NFUN_169__(Index, 0), __NFUN_170__(Index, 150)))
	{
		return;
	}
	// End:0x30
	...
	// End:0x6C
	if(__NFUN_174__(SelectedIndex, Index))
	{
		SelectedIndex = Index;
		GetPlayerOwner().PlaySoundInfo(0, class'UWindowScene'.default.SoundNavigateInfo);
	}
	ParentWindow.ScrollTo(0, __NFUN_195__(float(SelectedIndex), KeyHeight));
	ParentWindow.ScrollTo(0, __NFUN_198__(__NFUN_195__(float(SelectedIndex), KeyHeight), KeyHeight));
	return;
}
