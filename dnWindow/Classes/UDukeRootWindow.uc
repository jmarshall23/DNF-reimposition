/*******************************************************************************
 * UDukeRootWindow generated by Eliot.UELib using UELib.CLI.
...
function Scenes_Remove(int FirstIndex, int Length)
{
	...
	// End:0x3D [Loop If]
	if(__NFUN_169__(i, Length))
	{
		Scenes[__NFUN_165__(FirstIndex, i)].NotifyRemovingFromScenesList();
		__NFUN_182__(i);
		// [Loop Continue]
		goto J0x07;
	}
...
function ShowUWindowSystem(UWindow.UWindowRootWindow.EUWindowMode NewWindowMode)
{
	...
	// End:0x83
	if(__NFUN_148__(__NFUN_340__(Console, none), __NFUN_174__(int(NewWindowMode), int(9))))
	{
	...
	switch(NewWindowMode)
	{
		...
		case 1:
			...
			// End:0x134
			if(__NFUN_148__(__NFUN_414__(), __NFUN_145__(__NFUN_402__())))
			{
...
function bool CanShowAttractVideo()
{
	...
	// End:0xAD
	if(__NFUN_148__(__NFUN_148__(__NFUN_148__(__NFUN_148__(Desktop.IsStartupComplete(), __NFUN_339__(GetEntryLevel(), GetLevel())), __NFUN_145__(OnlineAgent.IsActive())), __NFUN_173__(string(DukeConsole(Console).DialogMgr.DialogStack), 0)), __NFUN_148__(__NFUN_170__(string(Scenes), 0), Scenes[__NFUN_166__(string(Scenes), 1)].AllowAttractVideo())))
	{
		return true;
	}
	return false;
...
function Tick(float Delta)
{
	...
	if(CanShowAttractVideo())
	{
		...
		// End:0x77
		if(__NFUN_201__(AttractTimer, 45))
		{
...
function UWindowScene LoadSceneFromCache(class<UWindowScene> WindowSceneClass)
{
	...
	// End:0x61 [Loop If]
	if(__NFUN_169__(i, string(SceneCache)))
	{
		// End:0x57
	...
	return SceneCache[__NFUN_166__(string(SceneCache), 1)];
	return;
}
...
function WindowEvent(UWindow.UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
	...
	{
		// End:0x3B
		if(__NFUN_170__(string(Scenes), 0))
		{
			Scenes[__NFUN_166__(string(Scenes), 1)].bUWindowActive = true;
		}
		// End:0x85
		if(__NFUN_173__(int(msg), int(10)))
		{
			...
			if(__NFUN_170__(string(Scenes), 0))
			{
				Scenes[__NFUN_166__(string(Scenes), 1)].WindowEvent(10, C, X, Y, Key);
			}
		}
		// End:0x146
		if(__NFUN_173__(int(msg), int(10)))
	...
	else
	{
		...
		else
		{
			...
			{
				Scenes[__NFUN_166__(string(Scenes), 1)].bUWindowActive = true;
				Scenes[__NFUN_166__(string(Scenes), 1)].WindowEvent(msg, C, X, Y, Key);				
			}
			else
			{
	...
	// End:0x353
	if(__NFUN_148__(__NFUN_173__(int(msg), int(10)), bNeedsMouseUpdateAfterPaint))
	{
...
function NavigateForward(class<UWindowScene> SceneClass)
{
	...
	Scenes[__NFUN_166__(string(Scenes), 1)].OnNavForward();
	FixUpUWindow();
...
{
	// End:0x3F
	if(__NFUN_170__(string(Scenes), 1))
	{
		Scenes_Remove(__NFUN_166__(string(Scenes), 1), 1);
		Scenes[__NFUN_166__(string(Scenes), 1)].OnNavReturn();
		FixUpUWindow();		
	}
	else
	{
...
{
	FirstChildWindow = Scenes[__NFUN_166__(string(Scenes), 1)];
	LastChildWindow = Scenes[__NFUN_166__(string(Scenes), 1)];
	// End:0x8A
	if(__NFUN_340__(Scenes[__NFUN_166__(string(Scenes), 1)].PrevSiblingWindow, none))
	{
		Scenes[__NFUN_166__(string(Scenes), 1)].PrevSiblingWindow.NextSiblingWindow = none;
		Scenes[__NFUN_166__(string(Scenes), 1)].PrevSiblingWindow = none;
	}
	return;
}
...
function string GetCustomizationTravelOptions()
{
	...
	// End:0x22F
	if(__NFUN_148__(__NFUN_340__(Player, none), __NFUN_340__(Player.PlayerProgress, none)))
	{
		...
		{
			// End:0x205
			if(__NFUN_173__(int(Player.PlayerProgress.GetChallengeStatus(ItemArray[i])), int(3)))
			{
...
function SendMapNamesToLCDHelper()
{
	...
	{
		// End:0x38
		if(__NFUN_170__(string(LCD.SPMapNames), 0))
		{
			return;
		}
		i = 0;
		...
		// End:0xA5 [Loop If]
		if(__NFUN_169__(i, string(class'UDukeSceneChapterSelect'.default.ChapterNames)))
		{
...
function OnJoinRoomFailed(int in_Reason)
{
	...
	// End:0x74
	if(__NFUN_148__(__NFUN_402__(), OnlineAgent.IsWaitingForResponse()))
	{
		DukeConsole(Console).DialogMgr.ShowWaitingForResponseDialog();
	}
	// End:0xB5
	if(__NFUN_174__(int(l_DialogType), int(0)))
	{
...
function OnInviteAccepted()
{
	...
	if(__NFUN_402__())
	{
		...
		// End:0xBB
		if(__NFUN_173__(int(ChatPermission), int(1)))
		{
...
function OnChatMessageReceived(string msg, string senderName)
{
	...
	// End:0x41 [Loop If]
	if(__NFUN_169__(i, string(Scenes)))
	{
...
{
	// End:0xBB
	if(__NFUN_150__(__NFUN_340__(DukeMultiPlayer(GetPlayerOwner()), none), DoesScreenExist('UDukeSceneMultiPlayerMenuLobby')))
	{
		OnlineAgent.DisconnectUnreal();
		Root.Console.bReturnToMainMenu = __NFUN_148__(__NFUN_145__(OnlineAgent.IsInMainMenu()), __NFUN_173__(int(ReturnTo), int(0)));
		Console.bReturnToMultiplayerMenu = __NFUN_173__(int(ReturnTo), int(1));
		// End:0xA4
		if(__NFUN_173__(int(ReturnTo), int(2)))
		{
			ShowUWindowSystem(1);			
		}
		else
...
function OnShutdown(int ErrorCode)
{
	...
	// End:0x73
	if(__NFUN_173__(int(l_DialogType), int(56)))
	{
		DukeConsole(Root.Console).DialogMgr.ShowWaitingDialog();
		return;
	}
	...
	{
		// End:0xA2
		if(__NFUN_150__(__NFUN_173__(int(l_DialogType), int(50)), __NFUN_173__(int(l_DialogType), int(46))))
		{
			ReturnToMainMenu(2);			
		}
		else
		{
			// End:0xCC
			if(__NFUN_148__(__NFUN_145__(DoesScreenExist('UDukeSceneLeaderboard')), __NFUN_145__(DoesScreenExist('UAgentSceneBrowserList'))))
			{
				ReturnToMainMenu(1);
			}
		}		
	...
	else
	{
		...
		{
			// End:0x121
			if(__NFUN_147__(GetPlayerOwner().IsMyDigs(), true))
			{
				ReturnToMainMenu(1);
			}
		}
	...
	{
		// End:0x1B5
		if(__NFUN_150__(__NFUN_145__(__NFUN_401__()), __NFUN_174__(int(l_DialogType), int(50))))
		{
			OnlineAgent.Breakpoint();
			DukeConsole(Console).DialogMgr.ShowDialogBox(l_DialogType);
		}
...
function OnLeaderboardError(int ErrorCode)
{
	...
	// End:0xDA
	if(__NFUN_150__(DoesScreenExist('UDukeSceneMultiplayer'), DoesScreenExist('UDukeSceneLeaderboard')))
	{
		DukeConsole(Console).DialogMgr.CloseAllDialogs();
		l_DialogType = OnlineAgent.TranslateShutdownReason(ErrorCode);
		// End:0x6E
		if(__NFUN_173__(int(l_DialogType), int(41)))
		{
			l_DialogType = 27;
		}
		// End:0xA8
...
function bool DoesTopSceneIgnoreInput()
{
	...
	if(__NFUN_170__(string(Scenes), 0))
	{
		return Scenes[__NFUN_166__(string(Scenes), 1)].bIgnoreAllInputs;
	}
	return false;
	return;
}
...
function MoveMouse(float X, float Y)
{
	...
	{
		// End:0x83
		if(__NFUN_170__(string(DukeConsole(Console).DialogMgr.DialogStack), 0))
		{
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
	...
	// End:0x96
	if(__NFUN_148__(__NFUN_173__(int(id), int(3)), __NFUN_173__(Result, 0)))
	{
		DukeConsole(Console).DialogMgr.ShowWaitingForResponseDialog();
		OnlineAgent.Cancel();
	}
...
function UDukeRootWindow.EOnlinePermissionCheck TickOnlinePermissionCheck(optional bool CheckChatPermission, optional bool bLogInOnly)
{
	...
	{
		// End:0x58
		if(__NFUN_145__(DukeConsole(Console).DialogMgr.IsDialogBoxActive(4)))
		{
			DukeConsole(Console).DialogMgr.ShowWaitingForResponseDialog();
		}
		return 2;
	...
	{
		// End:0xC2
		if(__NFUN_145__(LogInWasCalled))
		{
			LogInWasCalled = true;
			OnlineAgent.Login();
	...
	// End:0x1DD
	if(__NFUN_173__(int(OnlinePermission), int(0)))
	{
		// End:0x1D7
		if(CheckChatPermission)
		{
			...
			{
				// End:0x1A7
				if(__NFUN_173__(int(ChatPermission), int(2)))
				{
				...
				{
					// End:0x1D7
					if(__NFUN_173__(int(ChatPermission), int(3)))
					{
	...
	{
		// End:0x210
		if(__NFUN_173__(int(OnlinePermission), int(1)))
		{
		...
		{
			// End:0x243
			if(__NFUN_173__(int(OnlinePermission), int(2)))
			{
				DukeConsole(Console).DialogMgr.ShowDialogBox(52, self);				
			}
			else
			{
				// End:0x273
				if(__NFUN_173__(int(OnlinePermission), int(3)))
				{
					DukeConsole(Console).DialogMgr.ShowDialogBox(41, self);
				}
			}
