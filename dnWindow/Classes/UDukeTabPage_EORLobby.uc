/*******************************************************************************
 * UDukeTabPage_EORLobby generated by Eliot.UELib using UELib.CLI.
...
function OnNavForward()
{
	...
	{
		// End:0x287
		if(__NFUN_170__(GRI.Teams[RedTeamId].Score, GRI.Teams[BlueTeamId].Score))
		{
			RedTeamScoreSizeX = __NFUN_195__(class'MetagameHUD'.default.MaxTeamScoreScale, class'UWindowScene'.default.WinScaleX);
			RedTeamScoreSizeY = __NFUN_195__(class'MetagameHUD'.default.MaxTeamScoreScale, class'UWindowScene'.default.WinScaleY);
			BlueTeamScoreSizeX = __NFUN_195__(class'MetagameHUD'.default.MinTeamScoreScale, class'UWindowScene'.default.WinScaleX);
			BlueTeamScoreSizeY = __NFUN_195__(class'MetagameHUD'.default.MinTeamScoreScale, class'UWindowScene'.default.WinScaleY);
			WinString = __NFUN_302__(RedTeam, Wins);			
		}
		else
		{
			RedTeamScoreSizeX = __NFUN_195__(class'MetagameHUD'.default.MinTeamScoreScale, class'UWindowScene'.default.WinScaleX);
			RedTeamScoreSizeY = __NFUN_195__(class'MetagameHUD'.default.MinTeamScoreScale, class'UWindowScene'.default.WinScaleY);
			BlueTeamScoreSizeX = __NFUN_195__(class'MetagameHUD'.default.MaxTeamScoreScale, class'UWindowScene'.default.WinScaleX);
			BlueTeamScoreSizeY = __NFUN_195__(class'MetagameHUD'.default.MaxTeamScoreScale, class'UWindowScene'.default.WinScaleY);
			WinString = __NFUN_302__(BlueTeam, Wins);
		}
		// End:0x3B4
		if(__NFUN_173__(int(GetPlayerOwner().PlayerReplicationInfo.Team), RedTeamId))
		{
			RedTeamScoreX = __NFUN_195__(__NFUN_198__(class'MetagameHUD'.default.UnscaledScoreLeftX, ScoreOffset), class'UWindowScene'.default.WinScaleX);
			BlueTeamScoreX = __NFUN_195__(__NFUN_198__(class'MetagameHUD'.default.UnscaledScoreRightX, ScoreOffset), class'UWindowScene'.default.WinScaleX);			
		}
		else
		{
			BlueTeamScoreX = __NFUN_195__(__NFUN_198__(class'MetagameHUD'.default.UnscaledScoreLeftX, ScoreOffset), class'UWindowScene'.default.WinScaleX);
			RedTeamScoreX = __NFUN_195__(__NFUN_198__(class'MetagameHUD'.default.UnscaledScoreRightX, ScoreOffset), class'UWindowScene'.default.WinScaleX);
		}		
	}
	else
	{
...
function SetBackgroundBoundries(Canvas C)
{
	...
	{
		ScoreBoard.WinLeft = __NFUN_196__(__NFUN_211__(__NFUN_199__(float(C.SizeX), __NFUN_195__(float(C.SizeX), SBScaleX))), float(2));
		ScoreBoard.WinWidth = __NFUN_195__(float(C.SizeX), SBScaleX);
		ScoreBoard.WinTop = __NFUN_195__(210, class'UWindowScene'.default.WinScaleY);
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
	...
	// End:0x26F
	if(__NFUN_148__(__NFUN_173__(int(E), 2), SceneWindow.KeyButtons[0].WindowIsVisible()))
	{
		bPlayerLeft = UDukeScoreboardEntry(ScoreBoard.ChildInFocus).GetPRI().bPlayerLeft;
		// End:0x174
		if(__NFUN_148__(__NFUN_340__(C, none), __NFUN_340__(UDukeScoreboardEntry(C), none)))
		{
		...
		// End:0x26C
		if(__NFUN_148__(__NFUN_339__(C, SceneWindow.KeyButtons[KeyButton_RB]), __NFUN_145__(bPlayerLeft)))
		{
			MutePlayer();
		}		
	}
	else
	{
		...
		{
			// End:0x2EB
			if(__NFUN_150__(__NFUN_173__(int(E), 19), __NFUN_148__(__NFUN_173__(int(E), 2), __NFUN_339__(C, SceneWindow.KeyButtons[1]))))
			{
...
function KickPlayer()
{
	...
	bPlayerLeft = UDukeScoreboardEntry(ScoreBoard.ChildInFocus).GetPRI().bPlayerLeft;
	// End:0xD6
	if(__NFUN_150__(__NFUN_150__(__NFUN_145__(UDukeRootWindow(Root).AgentOnline.IsHost()), __NFUN_173__(roomMemID, CurrentPlayer.PlayerRef)), bPlayerLeft))
	{
		return;
	}
...
function MutePlayer()
{
	...
	bPlayerLeft = UDukeScoreboardEntry(ScoreBoard.ChildInFocus).GetPRI().bPlayerLeft;
	// End:0x107
	if(__NFUN_148__(__NFUN_148__(__NFUN_174__(roomMemID, CurrentPlayer.PlayerRef), __NFUN_174__(int(UDukeRootWindow(Root).AgentOnline.GetPlayerVoIPState(roomMemID)), int(0))), __NFUN_145__(bPlayerLeft)))
	{
		bMuted = UDukeRootWindow(Root).AgentOnline.IsPlayerMuted(roomMemID);
		UDukeRootWindow(Root).AgentOnline.MutePlayer(roomMemID, __NFUN_145__(bMuted));
	}
	return;
}
...
function Paint(Canvas C, float X, float Y)
{
	...
	bPlayerLeft = UDukeScoreboardEntry(ScoreBoard.ChildInFocus).GetPRI().bPlayerLeft;
	// End:0xD4
	if(__NFUN_150__(__NFUN_173__(roomMemID, CurrentPlayer.PlayerRef), bPlayerLeft))
	{
	...
	{
		// End:0x19E
		if(__NFUN_145__(SceneWindow.bIsHidingKeyButtons))
		{
			// End:0x13F
			if(__NFUN_174__(int(UDukeRootWindow(Root).AgentOnline.GetPlayerVoIPState(roomMemID)), int(0)))
			{
				updateMuteString();
	...
	CDFontScale = __NFUN_195__(TTFontScale, class'UDukeTabPage_EORReward'.default.CDTextScale);
	C.Font = C.TallFont;
	...
	if(__NFUN_148__(__NFUN_340__(GRI, none), GRI.bTeamGame))
	{
		Y = __NFUN_195__(class'MetagameHUD'.default.UnscaledScoreY, class'UWindowScene'.default.WinScaleY);
		...
		C.__NFUN_1250__(__NFUN_199__(RedTeamScoreX, __NFUN_196__(XL, 2)), __NFUN_199__(Y, __NFUN_196__(YL, 2)));
		...
		C.__NFUN_1250__(__NFUN_199__(BlueTeamScoreX, __NFUN_196__(XL, 2)), __NFUN_199__(Y, __NFUN_196__(YL, 2)));
	...
	ClipText(C, __NFUN_196__(__NFUN_199__(WinWidth, XL), 2), __NFUN_199__(__NFUN_199__(FillTopEdge, YL), __NFUN_195__(4, class'UWindowScene'.default.WinScaleX)), WinString,,,, 2);
	C.DrawColor = WhiteColor;
	// End:0x5A2
	if(__NFUN_170__(MPGameReplicationInfo(GetPlayerOwner().GameReplicationInfo).EORCountTime, 0))
	{
		...
		ClipText(C, __NFUN_199__(__NFUN_195__(float(C.SizeX), class'UDukeTabPage_EORReward'.default.CDXPos), CDXL), __NFUN_198__(ScoreBoard.WinTop, ScoreBoard.WinHeight), S,, CDFontScale, CDFontScale, 1, 2);
	}
	return;
}
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
	...
	{
		// End:0x159
		if(__NFUN_148__(__NFUN_173__(int(id), int(23)), __NFUN_173__(Result, 1)))
		{
