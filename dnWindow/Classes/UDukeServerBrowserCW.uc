/*******************************************************************************
 * UDukeServerBrowserCW generated by Eliot.UELib using UELib.CLI.
...
function Created()
{
	...
	Grid = UDukeServerGrid(VSplitter.CreateWindow(class'UDukeServerGrid', 0, 0, WinWidth, __NFUN_196__(WinHeight, float(2)), self));
	Grid.SetAcceptsFocus();
	InfoClient = UDukeInfoCW(VSplitter.CreateWindow(class'UDukeInfoCW', 0, 0, WinWidth, __NFUN_196__(WinHeight, float(2)), self));
	VSplitter.TopClientWindow = Grid;
	VSplitter.BottomClientWindow = InfoClient;
	VSplitter.SplitPos = __NFUN_195__(__NFUN_198__(Grid.WinHeight, InfoClient.WinHeight), 0.5);
...
function Resized()
{
	...
	Grid.SetSize(WinWidth, __NFUN_196__(WinHeight, float(2)));
	InfoClient.SetSize(WinWidth, __NFUN_196__(WinHeight, float(2)));
...
function QueryFinished(UDukeServerListFactory Fact, bool bSuccess, optional string ErrorMsg)
{
	...
	if(__NFUN_145__(bSuccess))
	{
		...
		// End:0x3E
		if(__NFUN_173__(UnpingedList.Count(), 0))
		{
			return;
		}		
	}
	...
	PingState = 2;
	// End:0x89
	if(__NFUN_148__(__NFUN_145__(bNoSort), __NFUN_145__(Fact.bIncrementalPing)))
	{
		PingedList.Sort();
	}
	UnpingedList.PingServers(true, __NFUN_150__(bNoSort, Fact.bIncrementalPing));
	return;
}
...
{
	// End:0x13
	if(__NFUN_145__(bHadInitialRefresh))
	{
		Refresh(false, true);
	}
	bPingSuspend = false;
...
function TestList()
{
	...
	// End:0x476 [Loop If]
	if(__NFUN_169__(i, 100))
	{
		// End:0x205
		if(__NFUN_173__(__NFUN_164__(i, 10), 0))
		{
			GameType = gamenames[__NFUN_187__(10)];
		}
		...
		NewListEntry.Ping = float(__NFUN_187__(9999));
		...
		NewListEntry.MapName = mapnames[__NFUN_187__(10)];
		NewListEntry.MapName = mapnames[__NFUN_187__(10)];
		NewListEntry.MapDisplayName = NewListEntry.MapName;
		NewListEntry.MaxPlayers = __NFUN_187__(64);
		NewListEntry.NumPlayers = __NFUN_188__(__NFUN_187__(64), NewListEntry.MaxPlayers);
		...
		// End:0x457 [Loop If]
		if(__NFUN_169__(j, NewListEntry.NumPlayers))
		{
			PlayerEntry = UDukePlayerList(NewListEntry.PlayerList.Append(class'UDukePlayerList'));
			PlayerEntry.PlayerID = j;
			PlayerEntry.PlayerName = playernames[__NFUN_187__(10)];
			// End:0x44D
			if(__NFUN_173__(__NFUN_187__(100), 0))
			{
...
function ApplyFilter()
{
	...
	if(__NFUN_340__(l, none))
	{
		l.bHidden = false;
		// End:0x61
		if(__NFUN_145__(BrowserWindow.Filter_Enabled))
		{			
		}
		else
		{
			// End:0xCD
			if(__NFUN_148__(__NFUN_309__(BrowserWindow.Filter_GameType, ""), __NFUN_309__(BrowserWindow.Filter_GameType, "All")))
			{
				// End:0xCD
				if(__NFUN_309__(l.GameType, BrowserWindow.Filter_GameType))
				{
					l.bHidden = true;
					// [Explicit Continue]
					goto J0x2DE;
			...
			// End:0x11B
			if(__NFUN_170__(BrowserWindow.Filter_MaxPing, 0))
			{
				// End:0x11B
				if(__NFUN_201__(l.Ping, float(BrowserWindow.Filter_MaxPing)))
				{
					l.bHidden = true;
					// [Explicit Continue]
					goto J0x2DE;
			...
			// End:0x168
			if(__NFUN_170__(BrowserWindow.Filter_MinPlayers, 0))
			{
				// End:0x168
				if(__NFUN_169__(l.NumPlayers, BrowserWindow.Filter_MinPlayers))
				{
					l.bHidden = true;
					// [Explicit Continue]
					goto J0x2DE;
			...
			// End:0x1B5
			if(__NFUN_170__(BrowserWindow.Filter_MaxPlayers, 0))
			{
				// End:0x1B5
				if(__NFUN_170__(l.NumPlayers, BrowserWindow.Filter_MaxPlayers))
				{
					l.bHidden = true;
					// [Explicit Continue]
					goto J0x2DE;
			...
			if(BrowserWindow.Filter_bUseBuddyList)
			{
				...
				if(__NFUN_169__(i, 32))
				{
					...
					if(__NFUN_340__(l.PlayerList, none))
					{
						...
						// End:0x2CE
						if(__NFUN_146__(l.bHidden, false))
						{
							// [Explicit Break]
							goto J0x2DE;
						}
...
function Refresh(optional bool bBySuperset, optional bool bInitial, optional bool bSaveExistingList, optional bool bInNoSort)
{
	...
	// End:0x2B
	if(__NFUN_145__(bSaveExistingList))
	{
		InfoItem = none;
		InfoClient.Server = none;
	}
	// End:0x6C
	if(__NFUN_148__(__NFUN_145__(bSaveExistingList), __NFUN_340__(PingedList, none)))
	{
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	P = PingState;
	// End:0x44
	if(__NFUN_173__(int(P), int(0)))
	{
		TotalReturnedServers = UnpingedList.Count();
	}
	// End:0x8C
	if(__NFUN_173__(int(P), int(3)))
	{
	...
	else
	{
		...
		TotalServers = __NFUN_165__(UnpingedList.Count(), PingedServers);
		MyServers = PingedList.Count();
		ShownServers = PingedList.ShownCount;
	}
	E = ErrorString;
	// End:0x11C
	if(__NFUN_170__(TotalServers, 0))
	{
		PercentComplete = int(__NFUN_196__(__NFUN_195__(float(PingedServers), 100), float(TotalServers)));
	}
	switch(P)
	{
		// End:0x174
		case 0:
			// End:0x166
			if(__NFUN_170__(TotalReturnedServers, 0))
			{
...
function Tick(float Delta)
{
	...
	// End:0x87
	if(__NFUN_170__(AutoRefreshTime, 0))
	{
		__NFUN_180__(TimeElapsed, int(Delta));
		// End:0x87
		if(__NFUN_170__(TimeElapsed, AutoRefreshTime))
		{
			TimeElapsed = 0;
