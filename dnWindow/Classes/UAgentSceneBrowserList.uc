/*******************************************************************************
 * UAgentSceneBrowserList generated by Eliot.UELib using UELib.CLI.
...
{
	super.Created();
	KeyButton_Filter = __NFUN_165__(3, 1);
	...
	Grid = UAgentBrowserGrid(CreateWindow(class'UAgentBrowserGrid', HorizBevel, VertBevel, __NFUN_199__(WinWidth, __NFUN_195__(HorizBevel, float(2))), __NFUN_199__(WinHeight, __NFUN_195__(VertBevel, float(2))), self));
...
function OnNavForward()
{
	...
	CurrentSortIdx = __NFUN_166__(6, 1);
	Factory.SortState = 1;
...
function Paint(Canvas C, float X, float Y)
{
	...
	super.Paint(C, X, Y);
	Grid.WinLeft = __NFUN_195__(float(C.SizeX), 0.09);
	Grid.WinTop = __NFUN_195__(float(C.SizeY), GridTopPct);
	Grid.WinWidth = __NFUN_199__(__NFUN_195__(float(C.SizeX), 0.925), Grid.WinLeft);
	Grid.WinHeight = __NFUN_195__(GridBottomPct, float(C.SizeY));
	DrawBackgroundBox(C, __NFUN_199__(Grid.WinLeft, float(10)), __NFUN_199__(Grid.WinTop, float(5)), Grid.WinWidth, Grid.WinHeight);
	topLineY = __NFUN_195__(28, WinScaleY);
	topLineX = __NFUN_195__(Grid.WinWidth, 0.1);
	DrawStretchedTexture(C, __NFUN_198__(Grid.WinLeft, topLineX), __NFUN_198__(Grid.WinTop, topLineY), __NFUN_195__(Grid.WinWidth, 0.8), 4, HGridLine, 1,,, true);
	return;
}

function DrawShoulderButtons(Canvas C)
{
	ShoulderButtons[0].WinLeft = __NFUN_195__(float(C.SizeX), 0.1);
	ShoulderButtons[0].WinTop = __NFUN_195__(float(C.SizeY), 0.1);
	ShoulderButtons[0].WinWidth = __NFUN_195__(150, WinScaleY);
	ShoulderButtons[0].WinHeight = __NFUN_195__(32, WinScaleY);
	ShoulderButtons[0].ButtonImageSize = 94;
	ShoulderButtons[1].WinWidth = __NFUN_195__(150, WinScaleY);
	ShoulderButtons[1].WinHeight = __NFUN_195__(32, WinScaleY);
	ShoulderButtons[1].WinLeft = __NFUN_199__(__NFUN_195__(float(C.SizeX), 0.9), ShoulderButtons[1].WinWidth);
	ShoulderButtons[1].WinTop = __NFUN_195__(float(C.SizeY), 0.1);
...
function RefreshServerList()
{
	...
	// End:0x1D
	if(__NFUN_173__(CurrentFilterModeIdx, FilterModeAll))
	{
		filterMode = -1;		
	}
	else
	...
	// End:0x5A
	if(__NFUN_173__(CurrentFilterMapIdx, FilterMapAll))
	{
		filterMap = -1;		
	}
	else
...
function ShowFilterDialog()
{
	...
	if(__NFUN_169__(i, string(class'MPMapInfo'.default.MapList)))
	{
		...
		// End:0x24E
		if(__NFUN_145__(bExist))
		{
			UDukeGBMessageBox(msgBox).AddFilterMapEntry(__NFUN_356__("MapNames", class'MPMapInfo'.default.MapList[i].MapName, "Maps"), string(i));
			__NFUN_182__(Count);
		}
		__NFUN_182__(i);
		// [Loop Continue]
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
	...
	if(__NFUN_173__(int(id), int(25)))
	{
		bFilterDialogVisible = false;
		// End:0x98
		if(__NFUN_148__(__NFUN_401__(), __NFUN_145__(UDukeRootWindow(Root).OnlineAgent.IsLoggedIn())))
		{
			return;
		}
		...
		// End:0x130
		if(__NFUN_174__(SortIdxReturned, CurrentSortIdx))
		{
			SetSortedColumn(SortIdxReturned);
		}
		// End:0x152
		if(__NFUN_174__(ModeIdxReturned, CurrentFilterModeIdx))
		{
			CurrentFilterModeIdx = ModeIdxReturned;
			bRefresh = true;
		}
		// End:0x174
		if(__NFUN_174__(MapIdxReturned, CurrentFilterMapIdx))
		{
			CurrentFilterMapIdx = MapIdxReturned;
			bRefresh = true;
		}
		// End:0x1B3
		if(__NFUN_174__(TypeIdxReturned, GetSearchConstraintForIndex()))
		{
		...
		{
			// End:0x200
			if(__NFUN_173__(TypeIdxReturned, 2))
			{
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
	...
	// End:0x6E
	if(__NFUN_148__(__NFUN_339__(C, KeyButtons[0]), __NFUN_173__(int(E), 2)))
	{
...
function Tick(float Delta)
{
	...
	{
		// End:0x92
		if(__NFUN_145__(UDukeRootWindow(Root).OnlineAgent.IsSignedIn()))
		{
		...
		{
			// End:0xEA
			if(__NFUN_148__(__NFUN_145__(GetPlayerOwner().__NFUN_971__(false)), __NFUN_145__(GetPlayerOwner().__NFUN_972__())))
			{
				DukeConsole(Root.Console).DialogMgr.CloseAllDialogs();
				NavigateBack();
			}
...
function NotifyScreenJoinRoomFailed(int in_Reason)
{
	...
	// End:0x75
	if(__NFUN_169__(failedServerIdx, string(Factory.Servers)))
	{
