/*******************************************************************************
 * TriggerAssignLightEx generated by Eliot.UELib using UELib.CLI.
...
event Trigger(Actor Other, Pawn EventInstigator)
{
	...
	// End:0x18
	if(__NFUN_148__(__NFUN_145__(bInterruptable), bActive))
	{
		return;
	}
	TrackedLights.Empty();
	...
	// End:0x40
	if(__NFUN_174__(string(TrackedLights), 0))
	{
		SetupTicking();
	}
	return;
...
final function SetupTicking()
{
	...
	FinalRedScaleString = __NFUN_319__(FinalRedScaleString);
	FinalGreenScaleString = __NFUN_319__(FinalGreenScaleString);
	FinalBlueScaleString = __NFUN_319__(FinalBlueScaleString);
	// End:0x7E
	if(__NFUN_201__(FinalScaleTotalTime, 0))
	{
		TimePerCharRed = __NFUN_196__(float(__NFUN_314__(FinalRedScaleString)), FinalScaleTotalTime);
		TimePerCharGreen = __NFUN_196__(float(__NFUN_314__(FinalGreenScaleString)), FinalScaleTotalTime);
		TimePerCharBlue = __NFUN_196__(float(__NFUN_314__(FinalBlueScaleString)), FinalScaleTotalTime);		
	}
	else
	{
		TimePerCharRed = 0;
...
final function CompletedLightString()
{
	...
	if(bRestoreLights)
	{
		...
		// End:0x6C [Loop If]
		if(__NFUN_169__(i, string(TrackedLights)))
		{
			// End:0x62
...
event Tick(float DeltaTime)
{
	...
	// End:0x2E
	if(__NFUN_203__(ElapsedTime, FinalScaleTotalTime))
	{
		CompletedLightString();
		return;
	}
	WorkingColor.R = __NFUN_330__(__NFUN_323__(FinalRedScaleString, int(__NFUN_195__(ElapsedTime, TimePerCharRed))));
	WorkingColor.G = __NFUN_330__(__NFUN_323__(FinalGreenScaleString, int(__NFUN_195__(ElapsedTime, TimePerCharGreen))));
	WorkingColor.B = __NFUN_330__(__NFUN_323__(FinalBlueScaleString, int(__NFUN_195__(ElapsedTime, TimePerCharBlue))));
	WorkingColorVector = __NFUN_232__(__NFUN_196__(float(WorkingColor.R), 255), __NFUN_196__(float(WorkingColor.G), 255), __NFUN_196__(float(WorkingColor.B), 255));
	i = __NFUN_166__(string(TrackedLights), 1);
	J0xE4:
