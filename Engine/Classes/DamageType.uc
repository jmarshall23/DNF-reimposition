/*******************************************************************************
 * DamageType generated by Eliot.UELib using UELib.CLI.
...
{
	// End:0x34
	if(__NFUN_148__(__NFUN_339__(DamagedPawn, DamageInstigator), __NFUN_145__(DamageInstigator.IsMP())))
	{
		return __NFUN_195__(default.SelfDamageScale, inDamage);
	}
	return inDamage;
	return;
}
...
static function ApplyClientDamageEffects(Pawn DamagedPawn, float Damage, Object.EPawnBodyPart BodyPart, name HitBoneName, Vector DamageOrigin, Vector DamageDirection, Vector DamageStart, float FrameDamage)
{
	...
	// End:0xE3
	if(__NFUN_148__(__NFUN_340__(DamagedPawn, none), DamagedPawn.bIsPlayerPawn))
	{
		PPawn = PlayerPawn(DamagedPawn);
		// End:0xA4
		if(__NFUN_201__(FrameDamage, 0))
		{
			// End:0x86
		...
		// End:0xE3
		if(__NFUN_150__(__NFUN_201__(Damage, 0), default.bShowIndicatorOnZeroDamage))
		{
...
static function SScreenFlash CreateScreenFlash(float Damage)
{
	...
	Intensity = __NFUN_224__(1, __NFUN_196__(Damage, default.DamageScreenFlashScaleDenominator));
	...
	Flash.Lifetime = __NFUN_228__(Intensity, default.DamageScreenFlashTimeMin, default.DamageScreenFlashTimeMax);
	Flash.LifetimeMid = __NFUN_195__(default.DamageScreenFlashTimeMidScale, Flash.Lifetime);
...
static function CreateBlurEffect(PlayerPawn Pawn, float Damage)
{
	...
	Intensity = __NFUN_227__(__NFUN_196__(Damage, default.MotionBlurDamageThreshold), 0, 1);
	Pawn.__NFUN_577__('Blur', 'Pain', 0, __NFUN_198__(0.1, __NFUN_195__(Intensity, 0.2)), __NFUN_198__(1, __NFUN_195__(Intensity, 2)));
	return;
}
...
static function CreateMotionBlurEffect(PlayerPawn Pawn, float Damage)
{
	...
	Intensity = __NFUN_227__(__NFUN_196__(__NFUN_199__(Damage, default.MotionBlurDamageThreshold), 75), 0, 1);
	Pawn.__NFUN_577__('MotionBlur', 'Pain', 0, __NFUN_198__(0.3, __NFUN_195__(Intensity, 0.3)), __NFUN_198__(3, __NFUN_195__(Intensity, 2)));
	return;
}
...
static function ApplyDamageVibration(PlayerPawn DamagedPawn, float Damage)
{
	...
	// End:0x112
	if(__NFUN_148__(__NFUN_340__(DamagedPawn, none), DamagedPawn.IsLocallyControlled()))
	{
		Index = __NFUN_166__(string(default.DamageVibrations), 1);
		J0x30:
		...
		{
			Index = __NFUN_166__(string(default.DamageRumbles), 1);
			J0xE2:
...
static function ApplyHUDDamageIndicator(PlayerPawn Player, float Damage, Vector DamageOrigin)
{
	...
	// End:0x48
	if(__NFUN_150__(__NFUN_150__(__NFUN_150__(__NFUN_339__(Player, none), __NFUN_339__(Player.MyHUD, none)), __NFUN_202__(Damage, 0)), __NFUN_202__(default.DamageIndicatorLifetime, 0)))
	{
		return;
	}
	// End:0x8F
...
static function CreateDamageViewKick(PlayerPawn Player, float Damage, Vector DamageStart)
{
	...
	// End:0x46
	if(__NFUN_203__(Damage, Player.DamageViewKickHardThreshold))
	{
	...
	vDir = __NFUN_253__(__NFUN_239__(DamageStart, Player.Location));
	FrontAmt = __NFUN_244__(vDir, Player.__NFUN_263__(__NFUN_232__(1, 0, 0), Player.Rotation));
	// End:0x15B
	if(__NFUN_206__(FrontAmt, 0))
	{
		ShakeInfo.ShakeName = 'DamageShake_Pitch';
		ShakeInfo.Style = 0;
		ShakeInfo.ShakeMagnitude = int(__NFUN_211__(__NFUN_195__(MaxKickAngle, FrontAmt)));
		// End:0x138
		if(__NFUN_201__(FrontAmt, 0))
		{
	...
	RightAmt = __NFUN_244__(vDir, Player.__NFUN_263__(__NFUN_232__(0, 1, 0), Player.Rotation));
	// End:0x21D
	if(__NFUN_206__(RightAmt, 0))
	{
		ShakeInfo.ShakeName = 'DamageShake_Roll';
		ShakeInfo.Style = 2;
		ShakeInfo.ShakeMagnitude = int(__NFUN_211__(__NFUN_195__(MaxKickAngle, RightAmt)));
		// End:0x1FA
		if(__NFUN_201__(RightAmt, 0))
		{
			ShakeInfo.Function = 5;			
		}
		else
...
{
	return __NFUN_196__(TestActor.DrawScale, TestActor.default.DrawScale);
	return;
}
...
defaultproperties
{
	...
	DamageIndicatorColor=(B=6,G=1,R=255,A=7)
