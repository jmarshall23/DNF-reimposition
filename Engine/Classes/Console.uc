/*******************************************************************************
 * Console generated by Eliot.UELib using UELib.CLI.
...
{
	BorderSize = __NFUN_191__(__NFUN_166__(BorderSize, 1), 0, 6);
	return;
}

exec function ViewDown()
{
	BorderSize = __NFUN_191__(__NFUN_165__(BorderSize, 1), 0, 6);
	return;
}
...
event Message(PlayerReplicationInfo PRI, coerce string msg, name n)
{
	...
	if(__NFUN_309__(msg, ""))
	{
		TopLine = __NFUN_164__(__NFUN_165__(TopLine, 1), 64);
		numLines = __NFUN_188__(__NFUN_165__(numLines, 1), __NFUN_166__(64, 1));
		MsgType[TopLine] = n;
		MsgTime = 6;
		__NFUN_184__(TextLines);
		// End:0x8F
		...
		MsgTick[TopLine] = __NFUN_198__(MsgTickTime, MsgTime);
	}
	return;
}
...
{
	// End:0x51
	if(__NFUN_148__(__NFUN_173__(int(Action), int(4)), MouseCapture))
	{
		switch(Key)
		{
			// End:0x35
...
event Tick(float Delta)
{
	...
	// End:0x4D
	if(__NFUN_174__(int(DeferredConsoleMessage), int(0)))
	{
		ShowMessageBox(int(DeferredConsoleMessage));
		DeferredConsoleMessage = 0;
	}
	// End:0x205
	if(__NFUN_150__(bWantsToPause, bWantsToUnpause))
	{
		// End:0x72
		if(bWantsToUnpause)
		{
		...
		// End:0x1F5
		if(__NFUN_148__(__NFUN_148__(__NFUN_148__(__NFUN_340__(Viewport, none), __NFUN_340__(Viewport.Actor, none)), __NFUN_340__(Viewport.Actor.Level, none)), __NFUN_340__(Viewport.Actor.Level.__NFUN_1161__(), none)))
		{
			// End:0x152
			if(__NFUN_148__(bWantsToPause, __NFUN_145__(Viewport.Actor.Level.bPaused)))
			{
				Viewport.Actor.Level.__NFUN_1161__().SetPause(true, "__Console", true);				
			}
			else
			{
				// End:0x1F5
				if(__NFUN_148__(__NFUN_148__(__NFUN_145__(bWantsToPause), Viewport.Actor.Level.bPaused), __NFUN_308__(Viewport.Actor.Level.Pauser, "__Console")))
				{
					Viewport.Actor.Level.__NFUN_1161__().SetPause(false, "__Console", true);
				}
			}
		}
		bWantsToPause = false;
	...
	{
		ConsolePos = __NFUN_224__(__NFUN_198__(ConsolePos, Delta), ConsoleDest);		
	}
	else
	{
		// End:0x258
		if(__NFUN_201__(ConsolePos, ConsoleDest))
		{
			ConsolePos = __NFUN_225__(__NFUN_199__(ConsolePos, Delta), ConsoleDest);
		}
	}
	// End:0x282
	if(__NFUN_148__(__NFUN_202__(__NFUN_210__(MsgTime, Delta), 0), __NFUN_170__(TextLines, 0)))
	{
		__NFUN_185__(TextLines);
	}
	// End:0x2C2
	if(__NFUN_148__(bTimerRunning, __NFUN_145__(Viewport.Actor.Level.bPaused)))
	{
		__NFUN_209__(MapperTimer, Delta);
	}
...
function TickLevelActionMessage()
{
	...
	{
		// End:0x74
		if(__NFUN_173__(int(Viewport.Actor.Level.LevelAction), int(4)))
		{
...
event PostRender(Canvas C)
{
	...
	if(bRestartTimeDemo)
	{
		...
		// End:0x66
		if(__NFUN_201__(__NFUN_199__(t, RestartTime), float(10)))
		{
			StartTimeDemo();
	...
	if(MouseCapture)
	{
		...
		{
			// End:0x10A
			if(__NFUN_203__(MouseX, float(C.SizeX)))
			{
				MouseX = float(__NFUN_166__(C.SizeX, 1));
			}
		}
		// End:0x124
		...
		{
			// End:0x157
			if(__NFUN_203__(MouseY, float(C.SizeY)))
			{
				MouseY = float(__NFUN_166__(C.SizeY, 1));
			}
		}
...
{
	// End:0x0D
	if(__NFUN_145__(bTimeDemo))
	{
		return;
	}
	bTimeDemo = false;
	// End:0x26
	if(__NFUN_145__(bRestartTimeDemo))
	{
		PrintTimeDemoResult();
	}
	return;
...
function PrintTimeDemoResult()
{
	...
	Delta = __NFUN_199__(__NFUN_199__(Level.TotalTimeSeconds, StartTime), ExtraTime);
	// End:0x5C
	...
	{
		Avg = __NFUN_196__(float(FrameCount), Delta);
	}
	...
	{
		temp = __NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(__NFUN_302__(FormatFloat(Avg), " Duke Nukem Forever v"), Viewport.Actor.Level.EngineVersion), __NFUN_321__(13)), __NFUN_321__(10)), FormatFloat(MinFPS)), " Min"), __NFUN_321__(13)), __NFUN_321__(10)), FormatFloat(MaxFPS)), " Max"), __NFUN_321__(13)), __NFUN_321__(10));
		SaveTimeDemo(temp);
	}
	return;
}
...
function TimeDemoCalc()
{
	...
	Delta = __NFUN_199__(Level.TotalTimeSeconds, LastFrameTime);
	__NFUN_184__(FrameCount);
	__NFUN_184__(LastSecondFrameCount);
	// End:0x160
	if(__NFUN_201__(__NFUN_199__(Level.TotalTimeSeconds, LastSecondStartTime), float(1)))
	{
		LastSecFPS = __NFUN_196__(float(LastSecondFrameCount), __NFUN_199__(Level.TotalTimeSeconds, LastSecondStartTime));
		// End:0x12A
		if(__NFUN_150__(__NFUN_204__(MinFPS, float(0)), __NFUN_200__(LastSecFPS, MinFPS)))
		{
			MinFPS = LastSecFPS;
		}
		// End:0x144
		if(__NFUN_201__(LastSecFPS, MaxFPS))
		{
			MaxFPS = LastSecFPS;
		}
		LastSecondFrameCount = 0;
...
function TimeDemoRender(Canvas C)
{
	...
	C.Font = C.LargeFont;
	C.Style = 1;
	t = __NFUN_199__(__NFUN_199__(Viewport.Actor.Level.TotalTimeSeconds, StartTime), ExtraTime);
	// End:0xA7
	...
	AText = __NFUN_303__(AvgText, FormatFloat(__NFUN_196__(float(FrameCount), t)));
	...
	C.__NFUN_1250__(__NFUN_199__(float(C.SizeX), __NFUN_198__(W, __NFUN_195__(15, PlayerHUD.CenterScale))), __NFUN_195__(float(C.SizeY), 0.3));
	...
	Y = __NFUN_198__(__NFUN_195__(float(C.SizeY), 0.3), h);
	C.__NFUN_1250__(__NFUN_199__(float(C.SizeX), __NFUN_198__(W, __NFUN_195__(15, PlayerHUD.CenterScale))), Y);
...
function TimerRender(Canvas C)
{
	...
	Min = int(__NFUN_196__(MapperTimer, 60));
	Hour = __NFUN_163__(Min, 60);
	__NFUN_181__(Min, __NFUN_162__(Hour, 60));
	Sec = __NFUN_199__(MapperTimer, __NFUN_195__(60, float(Min)));
	// End:0x95
	if(__NFUN_174__(Hour, 0))
	{
	...
	C.__NFUN_1250__(__NFUN_199__(__NFUN_199__(__NFUN_195__(float(PlayerHUD.DefaultResX), PlayerHUD.HUDScaleX), __NFUN_195__(PlayerHUD.HUDScaleX, W)), __NFUN_195__(float(16), PlayerHUD.HUDScaleX)), __NFUN_195__(__NFUN_195__(float(PlayerHUD.DefaultResY), PlayerHUD.HUDScaleY), 0.45));
