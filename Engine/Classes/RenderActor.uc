/*******************************************************************************
 * RenderActor generated by Eliot.UELib using UELib.CLI.
...
replication
{
	...
	// Pos:0x00B
	reliable if(__NFUN_148__(__NFUN_173__(int(Role), int(ROLE_Authority)), bIsPawn))
		HealthCap;
}
...
event PreBeginPlay()
{
	...
	// End:0x6B
	if(__NFUN_148__(IsMP(), __NFUN_170__(string(SpawnOnDestroyed), 0)))
	{
		// End:0x6B
		if(__NFUN_148__(SpawnOnDestroyed[0].SpawnClass.default.bIsDestroyedActorPool, __NFUN_173__(int(SpawnOnDestroyed[0].SpawnClass.default.Physics), int(18))))
		{
			bAlwaysRelevant = true;
		}
	}
	// End:0x9C
	if(__NFUN_201__(DukeVisionInterference, 0))
	{
...
simulated function InitializeAnimation()
{
	...
	{
		// End:0x36
		if(__NFUN_148__(__NFUN_340__(AnimationController, none), __NFUN_339__(AnimationController.Class, AnimationControllerClass)))
		{
			return;
		}
...
simulated event bool CreateActorPoolsOnDestroyed()
{
	...
	// End:0x0E
	if(__NFUN_170__(string(ActorPoolOnDestroyed), 0))
	{
		return false;
	}
	i = __NFUN_166__(string(SpawnOnDestroyed), 1);
	J0x1D:
	...
	{
		// End:0xB4
		if(__NFUN_148__(SpawnOnDestroyed[i].SpawnClass.default.bIsDestroyedActorPool, __NFUN_173__(int(SpawnOnDestroyed[i].SpawnClass.default.Physics), int(18))))
		{
...
simulated event PostNetInitial()
{
	...
	if(__NFUN_173__(int(Level.NetMode), int(NM_Client)))
	{
		...
		{
			// End:0x6F
			if(__NFUN_173__(int(default.MountOnSpawn[i].SpawnClass.default.RemoteRole), int(ROLE_None)))
			{
...
simulated function SetDamageInfo(float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
	...
	if(__NFUN_205__(ThisFrameDamage.DamageTime, Level.GameTimeSeconds))
	{
		...
		ThisFrameDamage.DamageOrigin = __NFUN_237__(__NFUN_238__(DamageOrigin, ThisFrameDamage.DamageOrigin), 2);
		__NFUN_249__(ThisFrameDamage.DamageDirection, DamageDirection);
		// End:0xAE
		if(__NFUN_148__(__NFUN_340__(ThisFrameDamage.DamageType, none), __NFUN_201__(DamageType.default.MomentumTransfer, ThisFrameDamage.DamageType.default.MomentumTransfer)))
		{
...
{
	return __NFUN_196__(Health, HealthCap);
	return;
}
...
simulated function Destroyed()
{
	...
	// End:0x2B
	if(__NFUN_148__(__NFUN_145__(bSilentDestroy), bSpawnOnDestroyed))
	{
		__NFUN_607__(GetSpawnOnDestroyedDelay(), false, 'SpawnOnDestroyedActivity');
	}
	// End:0x46
...
simulated function SpawnOnDestroyedActivity()
{
	...
	i = __NFUN_166__(string(SpawnOnDestroyedSimple), 1);
	J0x0F:
	...
	if(__NFUN_170__(string(ActorPoolOnDestroyed), 0))
	{
		...
		{
			// End:0x9E
			if(__NFUN_148__(__NFUN_169__(i, string(ActorPoolOnDestroyed)), __NFUN_340__(ActorPoolOnDestroyed[i], none)))
			{
	...
	i = __NFUN_166__(string(DestroyedParticleFriendEffects), 1);
	J0x104:
...
simulated function ExecuteParticleFriendEffect(out SParticleFriendEffect FriendEffect)
{
	...
	BaseRot = Rotation;
	// End:0xCE
	if(__NFUN_148__(__NFUN_340__(__NFUN_568__(), none), __NFUN_342__(FriendEffect.BoneName, 'None')))
	{
		BaseLoc = MeshInstance.__NFUN_533__(FriendEffect.BoneName, true);
		BaseRot = MeshInstance.__NFUN_534__(FriendEffect.BoneName, true);
	}
	// End:0x107
	if(__NFUN_145__(FriendEffect.bAbsoluteLocation))
	{
		Spawner.__NFUN_645__(__NFUN_238__(BaseLoc, __NFUN_263__(FriendEffect.Location, BaseRot)));		
	}
	else
	{
	...
	// End:0x150
	if(__NFUN_145__(FriendEffect.bAbsoluteRotation))
	{
		Spawner.__NFUN_652__(__NFUN_269__(FriendEffect.Rotation, BaseRot));		
	}
	else
	{
		Spawner.__NFUN_652__(FriendEffect.Rotation);
	}
	Spawner.SystemSizeScale = __NFUN_195__(Spawner.default.SystemSizeScale, __NFUN_196__(__NFUN_195__(FriendEffect.Scale, DrawScale), default.DrawScale));
...
final event ModifyHealth(float DeltaHealth, Vector ModifiedFrom)
{
	...
	else
	{
		...
		{
			TakeDamage(none, __NFUN_194__(DeltaHealth), Location, __NFUN_232__(1, 0, 0), class'CrushingDamage',, ModifiedFrom);
		}
	}
	return;
}
...
function ModifyHealthActivity(float DeltaHealth, Vector ModifiedFrom)
{
	...
	if(__NFUN_200__(DeltaHealth, float(0)))
	{
		...
		// End:0x1DC [Loop If]
		if(__NFUN_169__(i, string(DestructionLevels)))
		{
			// End:0x1D2
			if(__NFUN_148__(__NFUN_201__(Health, DestructionLevels[i].HealthReq), __NFUN_202__(__NFUN_198__(Health, DeltaHealth), DestructionLevels[i].HealthReq)))
			{
				// End:0x8A
				...
				j = __NFUN_166__(string(DestructionLevels[i].NewSkins), 1);
				J0xA4:
				...
				j = __NFUN_166__(string(DestructionLevels[i].SpawnAtLevelSimple), 1);
				J0x124:
				...
				j = __NFUN_166__(string(DestructionLevels[i].SpawnAtLevel), 1);
				J0x16C:
	...
	__NFUN_832__(__NFUN_198__(Health, DeltaHealth));
	return;
}

event ModifyHealthCap(float DeltaHealthCap)
{
	SetHealthCap(__NFUN_198__(HealthCap, DeltaHealthCap));
	return;
}

event RaiseHealthToCap(Vector ModifiedFrom)
{
	ModifyHealth(__NFUN_199__(GetHealthCap(), Health), ModifiedFrom);
	return;
}
...
{
	HealthMin = float(__NFUN_191__(int(NewHealthMin), 0, int(HealthCap)));
	__NFUN_832__(Health);
	return;
}
