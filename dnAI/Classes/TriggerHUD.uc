/*******************************************************************************
 * TriggerHUD generated by Eliot.UELib using UELib.CLI.
...
simulated function DrawAIHUD(Canvas C)
{
	...
	C.__NFUN_1250__(float(int(__NFUN_195__(float(C.SizeX), 0.005))), float(int(__NFUN_195__(float(C.SizeY), 0.055))));
...
final function PopulateTriggerList()
{
	...
	// End:0x0D
	if(__NFUN_145__(AutoPopulateTriggerList))
	{
		return;
	}
	// End:0x1D2
	foreach __NFUN_747__(class'TestingTrigger', t)
	{
		...
		if(__NFUN_342__(t.Group, 'None'))
		{
			...
			// End:0x168 [Loop If]
			if(__NFUN_169__(i, string(Groups)))
			{
				// End:0x15E
				if(__NFUN_341__(Groups[i].Group, t.Group))
				{
					...
					// End:0x117 [Loop If]
					if(__NFUN_169__(K, string(Groups[i].Triggers)))
					{
						// End:0x10D
						if(__NFUN_170__(Groups[i].Triggers[K].TriggerHUDIndex, t.TriggerHUDIndex))
						{
					...
					// End:0x15B
					if(__NFUN_173__(K, string(Groups[i].Triggers)))
					{
			...
			// End:0x1B7
			if(__NFUN_173__(i, string(Groups)))
			{
	...
	{
		// End:0x264
		if(__NFUN_170__(string(Groups[i].Triggers), 0))
		{
			// End:0x240
			if(__NFUN_170__(string(Groups[i].Triggers), 3))
			{
		...
		// End:0x2B8 [Loop If]
		if(__NFUN_169__(j, string(Groups[i].Triggers)))
		{
