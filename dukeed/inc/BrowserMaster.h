...
//			DebugBreak();
	...
	void RefreshBrowserTabs( INT InBrowser )
	{
		...
		for( INT x = 0 ; x < eBROWSER_MAX ; x++ )
			if( Browsers[x] && *Browsers[x] && (*Browsers[x])->IsDocked() )
				BrowserTabs->AddTab( (*Browsers[x])->Description.c_str(), (*Browsers[x])->BrowserID);
		...
		else
		{
			if( InBrowser != -1 )
			{
				...
				HMENU CurrentMenu = GetMenu( hWnd );
				DestroyMenu( CurrentMenu );
				SetMenu( hWnd, LoadMenuIdX(GetModuleHandle(NULL), (*Browsers[InBrowser])->MenuID));
	...
	INT FindBrowserIdxFromName( std::wstring InDesc )
	{
		for( INT x = 0 ; x < eBROWSER_MAX ; x++ )
