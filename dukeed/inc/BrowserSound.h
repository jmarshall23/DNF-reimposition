	...
	void OnCommand( INT Command )
	{
		switch( Command ) {
			...
			case IDMN_SB_EXPORT_WAV:
				{
// jmarshall - export all.
					for (int i = 0; i < pListSounds->GetCount(); i++)
					{
						OPENFILENAMEA ofn;
						char File[8192] = "\0";
						FString Name = pListSounds->GetString(i);
						FString fileName = TEXT("../newsounds/");
						fileName += Name;

						TCHAR l_chCmd[512];
						FString Package = pComboPackage->GetString(pComboPackage->GetCurrent());

						appSprintf(l_chCmd, TEXT("OBJ EXPORT TYPE=SOUND PACKAGE=\"%s\" NAME=\"%s\" FILE=\"%s.wav\""), *Package, *Name, *fileName);
						GEditor->Exec(l_chCmd);
