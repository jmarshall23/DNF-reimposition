		...
		case WM_KEYDOWN:
			switch (wParam)
			{
				case VK_RETURN:
				{
					...
					GCommand = Text;
					GEditor->exec.Exec(Text, (dnOutputDevice &)globalLog);
...
class WBottomBar : public WWindow
{
	...
	std::vector<WPictureButton *> PictureButtons;
	std::vector<WButton *> Buttons;
	HBITMAP hbm, hbmLogWnd;
	BITMAP bm;
	std::string MRUCommands[MAX_MRU_COMMANDS];
	INT NumMRUCommands;
	...
	{
		//hbmLogWnd = NULL;
		hbm = (HBITMAP)LoadImageA( GetModuleHandle(NULL), MAKEINTRESOURCEA(IDB_BOTTOM_BAR), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR);
		hbmLogWnd = (HBITMAP)LoadImageA(GetModuleHandle(NULL), MAKEINTRESOURCEA(IDB_BB_LOG_WND), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );	
	...
	void OnDestroy()
	{
		for (INT x = 0; x < PictureButtons.size(); x++)
		{
			DestroyWindow(PictureButtons[x]->hWnd);
			delete PictureButtons[x];
		}
	...
	void OnCreate()
	{
		...
		//LogCommandCombo->SelectionChangeDelegate = FDelegate(this,(TDelegate)&WBottomBar::OnLogCommandSelChange); // jmarshall: fix me
		...
		LogWndButton = new WButton(this, IDPB_LOG_WND);
		Buttons.push_back(LogWndButton);
		LogWndButton->OpenWindow( 1, 0, 0, 22, 20, NULL, 0, BS_OWNERDRAW );
		LogWndButton->SetBitmapReal( hbmLogWnd );
		
		SnapVertexCheck = new WPictureButton(this);
		PictureButtons.push_back(SnapVertexCheck);
		...
		DragGridCheck = new WPictureButton(this);
		PictureButtons.push_back(DragGridCheck);
		...
		RotGridCheck = new WPictureButton(this);
		PictureButtons.push_back(RotGridCheck);
		...
		MaximizeCheck = new WPictureButton(this);
		PictureButtons.push_back(MaximizeCheck);
	...
	void AddCommandToMRU( dnString InCommand )
	{
	...
	{
		GEditor->exec.Exec( LogCommandCombo->GetString2( LogCommandCombo->GetCurrent() ), (dnOutputDevice &)globalLog );
	}
};
