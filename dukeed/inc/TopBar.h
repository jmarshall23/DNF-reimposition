...
class WTopBar : public WWindow
{
	...
	void OpenWindow()
	{
		...
		for( INT x = 0 ; GTB_Buttons[x].ID != -1 ; x++ )
		{
			...
			{
				WButton* pButton = new WButton(this, GTB_Buttons[x].ID);
				dnString toolTipName(GTB_Buttons[x].ToolTip);

				pButton->OpenWindow(  true, Pos, VPos, TB_BUTTON_WIDTH, TB_BUTTON_HEIGHT, NULL, 0, BS_OWNERDRAW );
				GTB_Buttons[x].hbm = (HBITMAP)LoadImageA( GetModuleHandle(NULL), MAKEINTRESOURCEA(GTB_Buttons[x].BitmapID), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR);					
				pButton->SetBitmapReal(GTB_Buttons[x].hbm);
				ToolTipCtrl->AddTool( pButton->hWnd, toolTipName, GTB_Buttons[x].ID );
