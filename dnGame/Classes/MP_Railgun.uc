/*******************************************************************************
 * MP_Railgun generated by Eliot.UELib using UELib.CLI.
...
event PerformFire_Custom()
{
	...
	// End:0x1D
	if(__NFUN_174__(GetLoadedAmmo(), 0))
	{
...
simulated event SpawnWeaponTracer(Vector TracerStart, Vector TracerEnd, float TracerChance, class<BeamSystem> TracerBeamClass, class<SoftParticleSystem> TracerBubblesClass, Actor HitActor)
{
	...
	{
		TracerBeamSystem.BeamStartWidth = __NFUN_195__(__NFUN_196__(DrawScale, default.DrawScale), TracerBeamSystem.default.BeamStartWidth);
		TracerBeamSystem.BeamEndWidth = __NFUN_195__(__NFUN_196__(DrawScale, default.DrawScale), TracerBeamSystem.default.BeamEndWidth);
		TracerBeamSystem.AddSegment(TracerStart, TracerEnd);
	}
	// End:0x12C
	if(__NFUN_340__(TracerSmokeClass, none))
	{
		TracerSmokeClass.default.DieOutsideRadius = __NFUN_251__(__NFUN_239__(TracerEnd, TracerStart));
		TracerSmokeClass.default.SystemSizeScale = __NFUN_196__(DrawScale, default.DrawScale);
		__NFUN_615__(TracerSmokeClass, self,, TracerStart, Rotator(__NFUN_253__(__NFUN_239__(TracerEnd, TracerStart))), false, true);
	}
	return;
}
...
simulated event TraceFireMaterialHit(class<DamageType> TraceDamageType, Actor HitActor, Vector HitLocation, Vector HitNormal, class<Material> HitMaterial, bool bNoDecal)
{
	...
	{
		HitEffectActor.SystemSizeScale = __NFUN_196__(__NFUN_195__(HitEffectActor.default.SystemSizeScale, DrawScale), default.DrawScale);
