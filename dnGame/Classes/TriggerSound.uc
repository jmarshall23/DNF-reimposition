/*******************************************************************************
 * TriggerSound generated by Eliot.UELib using UELib.CLI.
...
{
	PlayVolume = __NFUN_225__(PlayVolume, 0);
	PlayPitch = __NFUN_227__(PlayPitch, 0, 4);
	PlayRadius = __NFUN_225__(PlayRadius, 0);
	PlayInnerRadius = __NFUN_227__(PlayInnerRadius, 0, PlayRadius);
	MinSoundPitch = __NFUN_227__(MinSoundPitch, 0.001, 4);
	MaxSoundPitch = __NFUN_227__(MaxSoundPitch, 0.001, 4);
	PlaySoundsUsed.Insert(0, string(PlaySounds));
	// End:0xA1
	if(__NFUN_173__(int(PlaySoundsStyle), int(2)))
	{
		RandomizePlaySounds();
	}
...
final simulated function RandomizePlaySounds()
{
	...
	// End:0x3A [Loop If]
	if(__NFUN_169__(i, string(PlaySounds)))
	{
	...
	// End:0x82 [Loop If]
	if(__NFUN_170__(string(temp), 0))
	{
		i = __NFUN_187__(string(temp));
...
{
	// End:0x18
	if(__NFUN_148__(bFadingIn, __NFUN_145__(bFadeInRestartable)))
	{
		return;
	}
	RefreshPlaySounds();
	// End:0x75
	if(__NFUN_148__(__NFUN_148__(__NFUN_145__(FadeOutOnlyOnTrigger), __NFUN_201__(FadeOutTime, 0)), __NFUN_170__(Loop, -1)))
	{
		__NFUN_607__(__NFUN_225__(1E-05, __NFUN_199__(__NFUN_195__(__NFUN_686__(PlaySound), float(__NFUN_189__(1, Loop))), FadeOutTime)), false, 'StartFadeOut');
	}
	// End:0xC3
	if(__NFUN_201__(FadeInTime, 0))
	{
		bFadingIn = true;
		TimeFadingIn = 0;
		// End:0xC0
		if(__NFUN_148__(bFadingOut, __NFUN_145__(bFadeInRestartable)))
		{
			__NFUN_209__(TimeFadingIn, __NFUN_195__(FadeInTime, SoundFadeFactor));
		}		
	}
	else
	{
...
function TS_PlaySound()
{
	...
	if(__NFUN_201__(FadeInTime, 0))
	{
		AdjustedVolume = __NFUN_195__(PlayVolume, __NFUN_196__(TimeFadingIn, FadeInTime));		
	}
	else
	{
	...
	if(PlayAsAmbient)
	{
		AmbientSoundInfo.Volume = byte(__NFUN_224__(__NFUN_195__(255, AdjustedVolume), 255));
	...
	else
	{
		...
		// End:0x11D
		if(__NFUN_173__(int(SoundPitchMode), int(0)))
		{
			// End:0xFC
			...
			__NFUN_607__(__NFUN_225__(1E-05, __NFUN_196__(TimeUntilSoundEnd, SoundInfo.Pitch)), false, 'DonePlaying');
		}
		SoundInfo.Volume = AdjustedVolume;
		// End:0x152
		if(__NFUN_145__(bUseReplication))
		{
...
{
	// End:0x1D
	if(__NFUN_148__(__NFUN_145__(bPlayingSound), __NFUN_173__(int(Role), int(ROLE_Authority))))
	{
		return;
	}
	// End:0x2C
	...
	// End:0x51
	if(__NFUN_148__(__NFUN_145__(StopImmediately), __NFUN_201__(FadeOutTime, 0)))
	{
		StartFadeOut();		
	}
	else
...
{
	// End:0x18
	if(__NFUN_148__(bFadingOut, __NFUN_145__(bFadeOutRestartable)))
	{
		return;
	}
	// End:0x26
	...
	// End:0x6B
	if(__NFUN_148__(bFadingIn, __NFUN_145__(bFadeOutRestartable)))
	{
		__NFUN_207__(TimeFadingOut, SoundFadeFactor);
	}
	UpdateTickState();
	return;
}
...
{
	__NFUN_607__(__NFUN_225__(1E-05, DonePlayingDelay), false, 'CallDonePlayingEvent');
	__NFUN_184__(LoopCount);
	bWaitingForDonePlaying = false;
	// End:0x4D
	if(__NFUN_150__(__NFUN_173__(Loop, -1), __NFUN_169__(LoopCount, Loop)))
	{
		TS_PlaySound();		
	}
	else
...
final function AssignSoundInfos()
{
	...
	{
		// End:0x3A
		if(__NFUN_171__(string(AmbientSoundInfo.AmbientSounds), 0))
		{
		...
		AmbientSoundInfo.Pitch = byte(__NFUN_224__(__NFUN_195__(64, PlayPitch), 255));
	...
	{
		// End:0x137
		if(__NFUN_171__(string(SoundInfo.Sounds), 0))
		{
		...
		SoundInfo.Pitch = __NFUN_227__(PlayPitch, 0.001, 4);
...
final function RefreshPlaySounds()
{
	...
	// End:0x19
	if(__NFUN_150__(bRefreshedNewSound, __NFUN_171__(string(PlaySounds), 0)))
	{
		return;
	}
...
final function int PickSound_DontRepeat(int PreviousIndex)
{
	...
	// End:0x0E
	if(__NFUN_173__(string(PlaySounds), 1))
	{
		return 0;
	}
	LastPlayed = PlaySounds[PreviousIndex];
	PlaySounds.Remove(PreviousIndex, 1);
	NewIndex = __NFUN_187__(string(PlaySounds));
...
{
	__NFUN_184__(PreviousIndex);
	// End:0x1E
	if(__NFUN_172__(PreviousIndex, string(PlaySounds)))
	{
		PreviousIndex = 0;
	}
...
{
	return __NFUN_187__(string(PlaySounds));
	return;
}
...
final function int PickSound_OnceOnly()
{
	...
	// End:0x0E
	if(__NFUN_173__(string(PlaySoundsUsed), 1))
	{
		return 0;
	}
	i = 0;
	...
	// End:0x51 [Loop If]
	if(__NFUN_169__(i, string(PlaySoundsUsed)))
	{
		// End:0x47
		if(__NFUN_173__(int(PlaySoundsUsed[i]), int(0)))
		{
			NewIndex = i;
			// [Explicit Break]
			goto J0x51;
	...
	// End:0xDA
	if(__NFUN_173__(i, string(PlaySoundsUsed)))
	{
		LastPlayed = PlaySounds[__NFUN_166__(string(PlaySounds), 1)];
		PlaySounds.Remove(__NFUN_166__(string(PlaySounds), 1), 1);
		RandomizePlaySounds();
		i = __NFUN_166__(string(PlaySoundsUsed), 1);
		J0x9B:
...
{
	// End:0x50
	if(__NFUN_150__(__NFUN_150__(__NFUN_150__(__NFUN_150__(bFadingIn, bFadingOut), __NFUN_148__(bPlayingSound, __NFUN_174__(int(SoundPitchMode), int(0)))), bForceTick), bVolRampForVelocity))
	{
		TickStyle = 2;		
	}
	else
...
event Tick(float Delta)
{
	...
	// End:0x83
	if(__NFUN_148__(bWaitingForDonePlaying, __NFUN_201__(TimeUntilSoundEnd, 0)))
	{
		__NFUN_210__(TimeUntilSoundEnd, __NFUN_195__(Delta, PlaySoundActor.__NFUN_688__(PlaySlot)));
		// End:0x83
		if(__NFUN_150__(__NFUN_202__(TimeUntilSoundEnd, 0), __NFUN_145__(PlaySoundActor.__NFUN_684__(PlaySlot))))
		{
			DonePlaying();
			return;
		}
...
final function UpdateVolume(float Delta)
{
	...
	if(bFadingIn)
	{
		...
		// End:0x3D
		if(__NFUN_203__(TimeFadingIn, FadeInTime))
		{
			bFadingIn = false;
			TimeFadingIn = FadeInTime;
			UpdateTickState();
		}
		SoundFadeFactor = __NFUN_196__(TimeFadingIn, FadeInTime);		
	}
	else
	{
		// End:0x9E
		if(bFadingOut)
		{
			__NFUN_210__(TimeFadingOut, Delta);
			// End:0x89
			...
			SoundFadeFactor = __NFUN_196__(TimeFadingOut, FadeOutTime);			
		}
		else
		{
			SoundFadeFactor = 1;
	...
	if(bVolRampForVelocity)
	{
		...
		VolRampVelocityScale = __NFUN_198__(VolRampVelocityScale, __NFUN_195__(__NFUN_199__(NewVolRampVelocityScale, VolRampVelocityScale), __NFUN_224__(1, __NFUN_195__(VolRampTransitionRate, Delta))));
		__NFUN_207__(SoundFadeFactor, VolRampVelocityScale);
	}
	// End:0x12F
	if(PlayAsAmbient)
	{
		PlaySoundActor.SoundVolume = byte(__NFUN_224__(__NFUN_195__(__NFUN_195__(255, PlayVolume), SoundFadeFactor), 255));		
	}
	else
	{
		PlaySoundActor.__NFUN_692__(PlaySlot, __NFUN_195__(PlayVolume, SoundFadeFactor));
	}
	return;
}
...
final function UpdatePitch(float DeltaSeconds)
{
	...
	// End:0xBD
	if(__NFUN_201__(PitchRampRate, 0))
	{
		// End:0x77
		if(PlayAsAmbient)
		{
			OldPitch = __NFUN_196__(float(PlaySoundActor.SoundPitch), 64);			
		}
		else
		{
			OldPitch = PlaySoundActor.__NFUN_688__(PlaySlot);
		}
		NewPitch = __NFUN_198__(OldPitch, __NFUN_195__(__NFUN_199__(NewPitch, OldPitch), __NFUN_224__(1, __NFUN_195__(PitchRampRate, DeltaSeconds))));
	}
	// End:0xF2
	if(PlayAsAmbient)
	{
		PlaySoundActor.SoundPitch = byte(__NFUN_227__(__NFUN_195__(NewPitch, 64), 0, 255));		
	}
	else
	{
...
final function float GetSoundPitchFromRotation()
{
	...
	{
		Rot = __NFUN_283__(MountParent.Rotation);		
	}
	else
	{
		Rot = __NFUN_283__(Rotation);
	}
	switch(SoundPitchMode)
	{
		// End:0x56
	...
	Angle = float(__NFUN_191__(int(Angle), int(MinValue), int(MaxValue)));
	NewSoundPitch = __NFUN_198__(MinSoundPitch, __NFUN_195__(__NFUN_196__(__NFUN_199__(Angle, MinValue), __NFUN_199__(MaxValue, MinValue)), __NFUN_199__(MaxSoundPitch, MinSoundPitch)));
	// End:0x114
	if(bDebugSoundPitch)
	{
...
final function float GetSoundPitchFromVelocity()
{
	...
	Speed = float(__NFUN_191__(int(Speed), int(MinValue), int(MaxValue)));
	NewSoundPitch = __NFUN_198__(MinSoundPitch, __NFUN_195__(__NFUN_196__(__NFUN_199__(Speed, MinValue), __NFUN_199__(MaxValue, MinValue)), __NFUN_199__(MaxSoundPitch, MinSoundPitch)));
	// End:0xBC
	if(bDebugSoundPitch)
	{
...
final function float GetSoundVolumeScale()
{
	...
	{
		// End:0x63
		if(__NFUN_203__(Speed, VolRampMaxVel))
		{
			return VolRampMaxScale;			
		}
		else
		{
			return __NFUN_198__(VolRampMinScale, __NFUN_195__(__NFUN_196__(__NFUN_199__(Speed, VolRampMinVel), __NFUN_199__(VolRampMaxVel, VolRampMinVel)), __NFUN_199__(VolRampMaxScale, VolRampMinScale)));
		}
	}
	return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
	...
	i = __NFUN_166__(string(PlaySounds), 1);
	J0x21:
