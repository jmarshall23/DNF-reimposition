/*******************************************************************************
 * dnControl generated by Eliot.UELib using UELib.CLI.
...
event TakeDamage(Pawn Instigator, float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
	...
	// End:0xC7
	if(__NFUN_148__(__NFUN_148__(__NFUN_340__(User, none), ShouldPassDamageToUser()), __NFUN_201__(UsableSomethingGetDamageScale(User, DamageType), 0)))
	{
		// End:0x7A
...
simulated function float UsableSomethingGetDamageScale(Pawn Pawn, class<DamageType> DamageType)
{
	...
	// End:0x6B
	if(__NFUN_148__(__NFUN_145__(Pawn.bIsPlayerPawn), __NFUN_203__(DamageScaleAI, 0)))
	{
...
function InitializeStateMachine()
{
	...
	// End:0xE6 [Loop If]
	if(__NFUN_169__(i, string(StateEvents)))
	{
		// End:0x3A
		if(__NFUN_145__(FindStateIndex(StateEvents[i].State, stateIndex)))
		{
			// [Explicit Continue]
			goto J0xDC;
		}
		// End:0x54
		if(__NFUN_173__(string(StateEvents[i].Events), 0))
		{
			// [Explicit Continue]
			goto J0xDC;
		}
		...
		// End:0xDC [Loop If]
		if(__NFUN_169__(j, string(StateEvents[i].Events)))
		{
			States[stateIndex].OutEvents[__NFUN_165__(StartIndex, j)] = StateEvents[i].Events[j];
			__NFUN_184__(j);
			// [Loop Continue]
			goto J0x83;
		}
...
simulated event PostUserTick(float DeltaTime)
{
	...
	// End:0x27
	if(__NFUN_150__(__NFUN_339__(InternalControlRemapper, none), __NFUN_339__(User, none)))
	{
		return;
	}
	// End:0x322
	if(User.__NFUN_841__(AnimPosDelta, AnimRotDelta, AnimPos, AnimRot))
	{
		...
		AnimRotDelta = __NFUN_283__(__NFUN_276__(AnimRot, User.Rotation));
		// End:0xB7
		if(__NFUN_148__(__NFUN_242__(AnimPosDelta, __NFUN_232__(0, 0, 0)), __NFUN_271__(AnimRotDelta, __NFUN_265__(0, 0, 0))))
		{
			return;
		}
		// End:0x1B7
		if(__NFUN_173__(int(InternalControlRemapper.CoreType), int(2)))
		{
			// End:0x105
			if(User.bIsPlayerPawn)
			{
				__NFUN_180__(User.ViewRotation.Yaw, AnimRotDelta.Yaw);				
			}
			else
			{
				User.__NFUN_652__(__NFUN_275__(User.Rotation, __NFUN_265__(0, AnimRotDelta.Yaw, 0)));
			}
			LinVelocity = __NFUN_237__(AnimPosDelta, DeltaTime);
			// End:0x19F
			if(__NFUN_173__(int(User.Physics), int(18)))
			{
				// End:0x188
		...
		else
		{
			...
			{
				InternalControlRemapper.MountOrigin = __NFUN_238__(UserMountPrefab.MountOrigin, User.MeshInstance.__NFUN_533__('movement'));
				InternalControlRemapper.MountAngles = __NFUN_275__(UserMountPrefab.MountAngles, User.MeshInstance.__NFUN_534__('movement'));
			...
			else
			{
				...
				InternalControlRemapper.__NFUN_652__(__NFUN_275__(InternalControlRemapper.Rotation, AnimRotDelta));
				FinalUserRot = __NFUN_269__(InternalControlRemapper.PlayerLockPrefab.MountAngles, InternalControlRemapper.Rotation);
				InternalControlRemapper.__NFUN_645__(__NFUN_238__(InternalControlRemapper.Location, __NFUN_241__(AnimPosDelta, FinalUserRot)));
...
simulated function CREvent_KeyChange(ControlRemapperEx CR, Pawn InputActor, Engine.Actor.EControlRemapperKeyType KeyType, Engine.Actor.EControlRemapperEventType EventType)
{
	...
	ControlEventInternal(__NFUN_165__(int(EnumBase), int(EventType)));
	return;
}
...
final simulated function SetHUDKeyInfoState(HUD HUD, Engine.Actor.EInteractKeyPosition Position, bool bEnable, optional Engine.Object.ESpecialKeys Key, optional bool bTap, optional bool bHighlight, optional string Description, optional float Scale)
{
	...
	// End:0x167
	if(__NFUN_170__(string(DisabledKeyTypes), 0))
	{
		switch(Key)
		{
			// End:0x32
		...
		{
			// End:0x15D
			if(__NFUN_173__(int(DisabledKeyTypes[i]), int(KeyTypeForKey)))
			{
				return;
			}
			__NFUN_185__(i);
...
function bool CanDetachPawn(bool bForceDetach)
{
	...
	// End:0x39
	if(__NFUN_150__(bForceDetach, __NFUN_170__(Index, -1)))
	{
...
simulated function CREvent_FullyAttached(ControlRemapperEx CR)
{
	...
	// End:0x72
	if(__NFUN_148__(__NFUN_173__(int(Role), int(ROLE_Authority)), __NFUN_340__(DamageTrigger, none)))
	{
		// End:0x72
		if(__NFUN_340__(DamageTrigger, none))
		{
...
simulated function bool SetState(int NewStateIndex)
{
	...
	// End:0x30
	if(__NFUN_172__(CurrentStateIndex, 0))
	{
		FinishState();
	}
	...
	StartState();
	// End:0x81
	if(__NFUN_148__(NotifyUserOnStateChange, __NFUN_340__(User, none)))
	{
...
simulated function int HasTransition(int ControlEvent, optional name CustomEventName)
{
	...
	i = __NFUN_166__(string(States[CurrentStateIndex].Transitions), 1);
	J0x1A:
	...
	if(__NFUN_172__(i, 0))
	{
		...
		// End:0x73
		if(__NFUN_148__(__NFUN_173__(int(Transition.ControlEvent), ControlEvent), __NFUN_341__(Transition.CustomName, CustomEventName)))
		{
			return i;
		}
		__NFUN_185__(i);
...
simulated function PlayUserAnim()
{
	...
	{
		// End:0x4E
		if(__NFUN_145__(User.SetUsableSomethingAnimation(UserAnimName)))
		{
		}
	}
	return;
...
simulated function PlayMyAnim()
{
	...
	{
		// End:0x63
		if(__NFUN_150__(__NFUN_339__(AnimationController, none), __NFUN_145__(AnimationController.SetAnimState(MyAnim.Animation))))
		{
...
function FireEvents(dnControl.EFirePoint FirePoint)
{
	...
	// End:0x93 [Loop If]
	if(__NFUN_169__(Index, string(States[CurrentStateIndex].OutEvents)))
	{
		OutEvent = States[CurrentStateIndex].OutEvents[Index];
		// End:0x5D
		if(__NFUN_148__(OutEvent.bPlayerUserOnly, __NFUN_339__(PlayerUser, none)))
		{
			// [Explicit Continue]
			goto J0x89;
		}
		// End:0x89
		if(__NFUN_173__(int(OutEvent.FirePoint), int(FirePoint)))
		{
...
function PrintStateMachine()
{
	...
	// End:0x246 [Loop If]
	if(__NFUN_169__(stateIndex, string(States)))
	{
		...
		// End:0x231 [Loop If]
		if(__NFUN_169__(TransitionIndex, string(States[stateIndex].Transitions)))
		{
...
event EnumerateRawAnimationSequences(out array<SAnimationEnumeration> References)
{
	...
	// End:0x71 [Loop If]
	if(__NFUN_169__(i, string(States)))
	{
		// End:0x67
...
state() AttachUserAnim
{
	...
	{
		// End:0xA9
		if(__NFUN_148__(__NFUN_174__(string(UsableExits), 1), __NFUN_174__(ActiveUsableExit, -1)))
		{
			// End:0x63
...
state() DetachUser
{
	simulated function StartState()
	{
		...
		global.StartState();
		// End:0x6B
		if(__NFUN_148__(__NFUN_145__(bUseHomePose), __NFUN_145__(bDelayedDetach)))
		{
			// End:0x65
			if(__NFUN_339__(User, none))
			{
				Warn(__NFUN_302__(string(self), ":DetachUser:StartState - User is None."));
				return;
			}
			AnimCallback_UserDetached();
		}
	...
	simulated function AnimCallback_UserReleased()
	{
		...
		// End:0x22
		if(__NFUN_148__(__NFUN_145__(bUseHomePose), bDelayedDetach))
		{
			AnimCallback_UserDetached();
		}
		return;
