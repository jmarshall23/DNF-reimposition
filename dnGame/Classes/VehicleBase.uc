/*******************************************************************************
 * VehicleBase generated by Eliot.UELib using UELib.CLI.
...
replication
{
	...
	// Pos:0x00B
	reliable if(__NFUN_148__(__NFUN_173__(int(Role), int(ROLE_Authority)), __NFUN_340__(CurrentDrivingVehicleSpace, none)))
...
{
	// End:0x11
	if(__NFUN_145__(IsHandbrakeEnabled()))
	{
		HandBrakeEnabled();
	}
	return;
...
{
	// End:0x1C
	if(__NFUN_148__(__NFUN_145__(bHandbrakeAlwaysOn), IsHandbrakeEnabled()))
	{
		HandBrakeDisabled();
	}
	return;
...
simulated event PostBeginPlay()
{
	...
	super.PostBeginPlay();
	i = __NFUN_166__(string(AdditionalInputHookControlRemappers), 1);
	J0x2F:
	...
	i = __NFUN_166__(string(AdditionalVehicleSpaces), 1);
	J0xAA:
	...
	{
		MyVehicleSensor.__NFUN_1125__(__NFUN_232__(VehicleSensorRadius, VehicleSensorRadius, VehicleSensorHeight));
	}
...
event KImpact(name SelfBoneName, KarmaActor Other, name OtherBoneName, Vector Position, Vector ImpactVelocity, Vector ImpactNormal)
{
	...
	// End:0x14D
	if(__NFUN_148__(Other.bIsPawn, __NFUN_174__(BlockedByPawnTickStamp, Level.LevelTickStamp)))
	{
		ThrottleRelative = GetThrottleInput();
		// End:0x14D
		if(__NFUN_206__(ThrottleRelative, 0))
		{
			Up = __NFUN_263__(__NFUN_232__(0, 0, 1), Rotation);
			// End:0xCB
			if(__NFUN_201__(ThrottleRelative, 0))
			{
				DriveDir = __NFUN_263__(__NFUN_232__(1, 0, 0), Rotation);				
			}
			else
			{
				DriveDir = __NFUN_263__(__NFUN_232__(-1, 0, 0), Rotation);
			...
			__NFUN_250__(Offset2D, __NFUN_235__(__NFUN_244__(Offset2D, Up), Up));
			// End:0x14D
			if(__NFUN_203__(__NFUN_244__(__NFUN_253__(Offset2D), DriveDir), 0.75))
			{
...
event TakeDamage(Pawn Instigator, float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
	...
	// End:0x79
	if(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace, none), CurrentDrivingVehicleSpace.ShouldReceiveDamageFromVehicle()))
	{
...
event bool TakePhysicsImpactDamage(float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, Pawn Instigator, KarmaActor DamageActor)
{
	...
	// End:0x8C
	if(__NFUN_150__(__NFUN_150__(__NFUN_150__(__NFUN_150__(__NFUN_339__(DamageActor, none), DamageActor.bStatic), DamageActor.bWorldGeometry), DamageActor.bIsPawn), DamageActor.bIsCorpse))
	{
		PassedDamageScale = 0;
...
function TriggerFunc_TogglePlayerLock()
{
	...
	if(__NFUN_172__(i, 0))
	{
		VehicleSpaces[i].bInternalUseDetach = __NFUN_145__(VehicleSpaces[i].bInternalUseDetach);
		VehicleSpaces[i].bPreserveInternalUseDetach = __NFUN_145__(VehicleSpaces[i].bPreserveInternalUseDetach);
		__NFUN_185__(i);
		// [Loop Continue]
		goto J0x0F;
	}
...
simulated function HandleVehicleEvents(VehicleBase.EVehicleEventType EventType)
{
	...
	i = __NFUN_166__(string(VehicleEvents), 1);
	J0x1F:
	...
	{
		// End:0xAC
		if(__NFUN_148__(__NFUN_173__(int(VehicleEvents[i].EventType), int(EventType)), __NFUN_345__(VehicleEvents[i].CompareType, float(NumFilledSpaces), VehicleEvents[i].Comparison)))
		{
...
final function bool RequestSpaceEntrance(VehicleSpaceBase Space, Actor Other)
{
	...
	// End:0x23
	if(__NFUN_148__(Space.bDriverSeat, __NFUN_340__(CurrentDrivingVehicleSpace, none)))
	{
		return false;
	}
	i = __NFUN_166__(string(VehicleSpaces), 1);
...
final function bool RequestSpaceExit(VehicleSpaceBase Space, Actor Other)
{
	...
	// End:0x6C
	if(__NFUN_150__(__NFUN_169__(i, 0), __NFUN_340__(VehicleSpaces[i].User, Other)))
	{
		return false;
	}
...
simulated event DriverLeft(VehicleSpaceBase Space)
{
	...
	{
		// End:0x6D
		if(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace.User, none), __NFUN_340__(CurrentDrivingVehicleSpace.User.AnimationController, none)))
		{
...
simulated event Tick(float DeltaTime)
{
	...
	if(__NFUN_340__(CurrentDrivingVehicleSpace, none))
	{
		...
		{
			GravityScale = __NFUN_195__(default.GravityScale, GravityRange.Min);			
		}
		else
		{
			// End:0xA4
			if(__NFUN_150__(__NFUN_201__(DeltaTime, GravityTimestepRange.Max), __NFUN_204__(GravityTimestepRange.Min, GravityTimestepRange.Max)))
			{
				GravityScale = __NFUN_195__(default.GravityScale, GravityRange.Max);				
			}
			else
			{
				GravityScale = __NFUN_195__(default.GravityScale, __NFUN_228__(__NFUN_196__(__NFUN_199__(GravityTimestepRange.Max, DeltaTime), __NFUN_199__(GravityTimestepRange.Max, GravityTimestepRange.Min)), GravityRange.Max, GravityRange.Min));
			}
		}		
	}
	else
...
simulated function Tick_Driver(float DeltaTime)
{
	...
	// End:0x45
	if(__NFUN_174__(int(CurrentDrivingVehicleSpace.User.Role), int(ROLE_Authority)))
	{
		TickDriving(DeltaTime);
	}
	// End:0x6F
	if(__NFUN_145__(CurrentDrivingVehicleSpace.User.bIsPlayerPawn))
	{
		TickDriving(DeltaTime);
	}
	// End:0xDF
	if(__NFUN_148__(__NFUN_201__(DrivenAvoidRange, 0), CurrentDrivingVehicleSpace.User.bIsPlayerPawn))
	{
		// End:0xBC
		if(__NFUN_203__(__NFUN_211__(oForwardVelocity), 300))
		{
			AvoidRange = DrivenAvoidRange;			
		}
		else
		{
			AvoidRange = __NFUN_228__(__NFUN_196__(__NFUN_211__(oForwardVelocity), 300), DrivenAvoidRangeMin, DrivenAvoidRange);
		}		
	}
	else
	{
...
simulated function bool TickDriving(float DeltaTime)
{
	...
	// End:0x1C
	if(__NFUN_150__(__NFUN_173__(string(VehicleSpaces), 0), __NFUN_339__(CurrentDrivingVehicleSpace, none)))
	{
		return false;
	}
	// End:0x3E
	if(__NFUN_145__(CurrentDrivingVehicleSpace.SpaceIsFullyEntered()))
	{
...
{
	// End:0x2A
	if(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace, none), __NFUN_145__(bEngineForcedOff)))
	{
...
simulated function TickDriving_Throttle(float DeltaTime, Pawn P)
{
	...
	if(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace, none), __NFUN_145__(bEngineForcedOff)))
	{
		...
		{
			// End:0x6E
			if(__NFUN_148__(__NFUN_148__(__NFUN_145__(IsBoosting()), CurrentDrivingVehicleSpace.bBoost), __NFUN_145__(bWantsToExit)))
			{
				BeginBoost();				
			}
			else
			{
				// End:0x9F
				if(__NFUN_150__(__NFUN_148__(IsBoosting(), __NFUN_145__(CurrentDrivingVehicleSpace.bBoost)), bWantsToExit))
				{
					EndBoost();
				}
			}			
	...
	// End:0x13F
	if(__NFUN_150__(__NFUN_204__(ThrottleRelative, 0), bWantsToExit))
	{
		// End:0xF3
		if(__NFUN_148__(__NFUN_204__(ThrottleRelative, 0), bWantsToExit))
		{
		...
		{
			// End:0x132
			if(__NFUN_148__(__NFUN_201__(ThrottleRelative, 0), bCanResumeDriving))
			{
...
simulated function TickDriving_Heading(float DeltaTime, Pawn P)
{
	...
	// End:0x57
	if(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace, none), __NFUN_150__(__NFUN_145__(bEngineForcedOff), __NFUN_145__(bDisableAllWhenEngineOff))))
	{
		switch(ControlType)
		{
			// End:0x36
...
simulated function TickDriving_ZMotion(float DeltaTime, Pawn P)
{
	...
	switch(ZMotionType)
	{
		...
		// End:0x9E
		case 1:
			ZMotion_AddAbsolute(__NFUN_195__(__NFUN_195__(DeltaTime, ZMotionScale), ZMotionPower));
			// End:0xC2
			break;
		// End:0xBF
		case 2:
			ZMotion_AddThruster(__NFUN_195__(__NFUN_195__(DeltaTime, ZMotionScale), ZMotionPower));
			// End:0xC2
			break;
		// End:0xFFFF
		default:
...
{
	// End:0x12
	if(__NFUN_145__(CanToggleHeadlights(Toggler)))
	{
		return false;
	}
	// End:0x42
...
function ForcePowerOn()
{
	...
	// End:0x4A
	if(__NFUN_148__(__NFUN_145__(bIgnition), __NFUN_340__(VehicleGetDriver(), none)))
	{
		TurnIgnitionOn();
	}
	return;
...
{
	NavOrigin = Location;
	__NFUN_209__(NavOrigin.Z, __NFUN_199__(CollisionHeight, GetVehicleFloorHeight()));
	return;
}
...
simulated function VehicleKilledPawn(Corpse Corpse, Vector HitLocation)
{
	...
	// End:0x146
	if(__NFUN_148__(__NFUN_148__(__NFUN_340__(CurrentDrivingVehicleSpace, none), __NFUN_340__(CurrentDrivingVehicleSpace.User, none)), CurrentDrivingVehicleSpace.User.bIsPlayerPawn))
	{
		PP = PlayerPawn(CurrentDrivingVehicleSpace.User);
		PP.AddRumble(KilledPawnRumble);
		// End:0x93
		if(__NFUN_200__(__NFUN_222__(), VehicleKillLineChance))
		{
			PP.FindAndPlaySound(VehicleKillLine);
		}
		CurrentDrivingVehicleSpace.User.ShakeView(KilledPawnViewShake, true);
		// End:0x146
		if(__NFUN_206__(BloodSplatterTime, Level.GameTimeSeconds))
		{
			...
			{
				BloodScreenSplatterEffect.__NFUN_645__(__NFUN_238__(PP.CalcedViewLocation, Vector(PP.CalcedViewRotation)));
				BloodScreenSplatterEffect.AttachEffectTo(PP);
			}
		}
	}
	// End:0x355
	if(__NFUN_148__(__NFUN_340__(Corpse, none), __NFUN_340__(Corpse.__NFUN_568__(), none)))
	{
		// End:0x1CB
		if(__NFUN_340__(BloodMessClass, none))
		{
			BloodyMess = __NFUN_615__(BloodMessClass);
			// End:0x1CB
			if(__NFUN_340__(BloodyMess, none))
			{
				BloodyMess.MountType = 2;
				BloodyMess.MountMeshItem = 'Head';
				BloodyMess.__NFUN_635__(Corpse, false, false, false, true);
			}
		}
		Impulse = __NFUN_234__(__NFUN_798__(HitLocation), __NFUN_198__(1, __NFUN_198__(ForwardCorpseBoost, __NFUN_195__(__NFUN_222__(), ForwardCorpseBoostRnd))));
		Up = __NFUN_233__(__NFUN_253__(__NFUN_640__()));
		__NFUN_250__(Impulse, __NFUN_235__(__NFUN_198__(UpwardCorpseBoost, __NFUN_195__(__NFUN_222__(), UpwardCorpseBoostRnd)), __NFUN_640__()));
		...
		SideScale = __NFUN_244__(__NFUN_253__(__NFUN_239__(Corpse.Location, HitLocation)), Right);
		// End:0x2BF
		...
		Corpse.__NFUN_790__(__NFUN_235__(ImpulseMag, Impulse), 1);
		Corpse.__NFUN_821__(__NFUN_263__(__NFUN_235__(1000, __NFUN_256__(__NFUN_232__(1875, 1875, 1875), __NFUN_232__(500, 500, 500))), Corpse.Rotation));
	}
...
function RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
	...
	// End:0x31
	if(__NFUN_201__(VehicleKillLineChance, 0))
	{
...
defaultproperties
{
	...
	UsePostureFailure=/* Array type was not detected. */
	X=true
